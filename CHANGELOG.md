...
- **Policy Authoring Diagnostic Surface Retirement** — removed the obsolete
  migration-verifier authoring component, accessibility surface, readiness
  exemption, and workflow role; readiness now rejects retired diagnostic
  identifiers rather than preserving a hidden alternate authoring path.
...
- **Classification Evidence Migration Vocabulary** — replaced delivery-era
  ...
  naming baseline from `3/4` to `0/1` references/candidates.
...
- **Policy Request-Time Learning Input Boundary** — separated raw request-event
  ...
  provenance derived only from the clarification contract.
...
- **Policy Intent Entry Normalizer** — added a server-owned primitive-field
  ...
  excluded or audited before later policy engine stages consume intent.
...
- **Policy Authoring Accessibility Cutover** — renamed the accessibility and
  ...
  `POLICY_AUTHORING_ACCESSIBILITY_*` and
  `policyAuthoringAccessibility*`, updated completion-audit records to track
  `policy_authoring_accessibility`, and preserved label/helper, keyboard/focus,
  multi-select, chip-removal, disabled-reason, single-next-action, and
  diagnostic-exclusion validation behavior.
...
- **Policy Request-Time Learning Module Cutover** — renamed the request-time
  ...
  replaced the contract-local phase handoff with `nextStep`, updated downstream
  runtime consumers, and retained the completion-audit adapter for the
  library-policy-rebuild handoff.
...
- **Policy Automation Decision Contract Module Cutover** — renamed the runtime
  ...
  contract-local phase handoff with `nextStep`, updated downstream runtime
  consumers, and retained the completion-audit adapter for the runtime question
  reduction handoff.
...
- **Policy Evidence Input Gate Module Cutover** — renamed the evidence input
  ...
  with `nextStep`, updated boundary imports, and lowered the production naming
  regression baseline after inventory validation.
...
- **Policy Learning Quality Gate** — hardened bounded learning so the policy
  ...
  candidates.
- **Policy Intent Quality Gate** — hardened bounded intent inference so the
  policy intent engine consumes generated evidence quality, blocks missing or
  insufficient evidence-quality handoffs, and carries sanitized quality
...
- **Policy Builder Phase 7R Completion Audit** — added a server-owned
  ...
  Phase 8R native storage work begins.
...
- **Policy Builder Phase 7R Migration Verifier Validation Freshness** —
  ...
  must match guarded-outcome fingerprint and request-proof counts.
...
- **Policy Builder Phase 7R Request Learning Evidence Binding** — hardened
  ...
  learning-guard context, and trace, with validation rejecting missing or
  mismatched handoffs.
- **Policy Builder Phase 7R Question Evidence Binding** — hardened runtime
  question reduction so question plans carry the automation decision evidence
  fingerprint into the plan, planned question, and bounded trace attributes, with
  validation rejecting missing or mismatched bindings.
