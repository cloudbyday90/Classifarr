...
- **Task Queue: Composite Partial Index for `dequeue()`** — Migration `20260305_100000_optimize_task_queue_indexes.sql` adds `idx_task_queue_dequeue` on `task_queue (next_retry_at ASC, priority DESC, created_at ASC) WHERE status = 'pending'`. This matches the exact `WHERE` + `ORDER BY` pattern of `QueueService.dequeue()` so the planner can satisfy the filter and sort entirely from the index without a sequential scan. A second index `idx_task_queue_processing_stale` on `(started_at) WHERE status = 'processing'` speeds up `resetStaleProcessingTasks()` and `gracefulShutdown()`. Both created with `CONCURRENTLY` (no `AccessExclusiveLock` on live instances) and `IF NOT EXISTS` (idempotent).
