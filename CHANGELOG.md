...
- **`pg_advisory_xact_lock` for Backfill Services** — `tryAdvisoryLock(client, lockKey)` and `DB_ADVISORY_LOCKS` constants (`IDLE_BACKFILL=1001`, `SCHEDULED_BACKFILL=1002`, `MANUAL_BACKFILL=1003`) added to `database.js`. All three backfill services (`idleBackfillService`, `scheduledBackfillService`, `manualBackfillService`) now acquire a transaction-scoped advisory lock during startup to prevent split-brain races between processes. Lock auto-releases on transaction commit; no leak risk on crash.
...
- **Migration `20260305_110000_add_security_cleanup_indexes`** — Adds `idx_refresh_tokens_expires_at` and `idx_api_key_audit_created_at` to make the new cleanup jobs index-efficient.
- **Migration `20260305_120000_add_classification_history_check_constraints`** — Adds `chk_classification_confidence_range` (`confidence` 0–100 or NULL) and `chk_classification_completed_has_library` (`status='completed'` requires non-NULL `library_id`) as `NOT VALID` constraints on `classification_history`. Instant DDL; does not lock existing rows.
- **Migration `20260305_130000_validate_classification_history_constraints`** — Validates both constraints added in the previous migration using `VALIDATE CONSTRAINT` (SHARE UPDATE EXCLUSIVE lock; reads and writes continue). After validation, PostgreSQL can use constraints for query optimizations.
...
- **`ragRetriever.test.js`** — Added `'HNSW ef_search tuning'` describe block (4 tests: `SET LOCAL` issued before CTE, defaults to 80, respects `PGVECTOR_EF_SEARCH` env var, client released on error).
