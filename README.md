...
![Version](https://img.shields.io/badge/version-v0.45.6--beta-blue.svg)
...
**v0.45.6-beta** is the current public release label in this repo. Package files use the semver-safe form `0.45.6-beta`, while the UI, release notes, and Git tags use `v0.45.6-beta`. Docker Hub reports **15,727 pulls** for `cloudbyday90/classifarr` as of 2026-04-26. This beta line is positioned as stable and production-capable for self-hosted media library operators.
...
services:
  classifarr:
    image: ghcr.io/cloudbyday90/classifarr:v0.45.6-beta
...
Current status: cookie-security wrapper cleanup and the latest ESM mock-shape cleanup tranche are done, CI includes a multiline-aware guard for synthetic named-service default-wrapper test mocks (`npm run esm:check-test-mock-shapes`, baseline currently 0), strict-mode scanning for any named+default synthetic wrappers is available (`npm run esm:check-test-mock-shapes:strict`) with optional category summaries (`npm run esm:report-test-mock-shapes:strict:summary`), strict backlog is currently 20 candidates with logger/auth categories fully burned down in this tranche, shared `createNamedServiceStub(...)` helper extraction is now in use across route/integration suites to reduce mock drift, CI emits a JSON artifact report via `npm run esm:report-test-mock-shapes:artifact` and uploads it in GitHub Actions, and the current server/client dependency check returned no outdated packages from the locked graph.
