...
Current status: cookie-security wrapper cleanup and the latest ESM mock-shape cleanup tranche are done, CI now includes a baseline-backed guard for new synthetic named-service default-wrapper test mocks (`npm run esm:check-test-mock-shapes`, baseline currently 0), and the current server/client dependency check returned no outdated packages from the locked graph.
