    ...
    <template v-else>
      ...
      <Card>
        <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
          <div>
            <label class="block text-sm font-medium text-gray-300 mb-2">Target Library</label>
...
export default {
  ...
  setup() {
    ...
    const loadSummary = async () => {
      try {
        const response = await api.getPatternSummary();
    ...
    const loadPatterns = async () => {
      try {
        ...
        const response = await api.getPatterns(params);
    ...
    const viewDetails = async (pattern) => {
      try {
        ...
        const response = await api.getPattern(pattern.id);
    ...
    const rejectPattern = async (id) => {
      try {
        await api.rejectPattern(id, { rejected_by: 'user', rejection_reason: 'Manual rejection' });
        await loadPatterns();
        await loadSummary();
