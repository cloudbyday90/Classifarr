...
<template>
  <div class="space-y-6">
    ...
    <div class="bg-gray-800 border border-gray-700 rounded-lg p-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div>
          ...
          >
          <p class="mt-1 text-xs text-gray-500">
            Timeout for embedding requests (default: 30000)
          </p>
        </div>
      </div>
    </div>
    ...
    <div class="bg-gray-800 border border-gray-700 rounded-lg p-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <div>
          ...
          >
          <p class="mt-1 text-xs text-gray-500">
            Extended timeout for cold model (default: 120000ms / 120s)
          </p>
        </div>
        ...
        <div>
          ...
          >
          <p class="mt-1 text-xs text-gray-500">
            Normal timeout for warm model (default: 30000ms / 30s)
          </p>
        </div>
        ...
        <div>
          ...
          >
          <p class="mt-1 text-xs text-gray-500">
            Randomization factor (0-1, default: 0.3 for ±30%)
          </p>
        </div>
      </div>
      ...
      <div class="bg-gray-900 rounded-lg p-4 mb-4">
        ...
        <div class="space-y-2">
          <div
            v-for="(delay, i) in exampleBackoffSequence"
            :key="i"
    ...
    <div class="bg-gray-800 border border-gray-700 rounded-lg p-6">
      ...
      <div
        v-if="!secondPassConfigAvailable"
        class="bg-yellow-900/20 border border-yellow-500/40 rounded-lg p-3 text-sm text-yellow-300"
      >
      ...
      >
        ...
        <div class="flex items-center justify-between p-3 bg-gray-700/30 rounded-lg">
          <div>
            <p class="font-medium text-white">
              Policy prompt-threshold re-check
            </p>
            <p class="text-sm text-gray-400">
              Enable targeted policy re-check when prompt-threshold confidence is not met
            </p>
          </div>
          <label class="relative inline-flex items-center cursor-pointer">
            <input
        ...
        <div
          ...
          :class="secondPassConfig.rag_loop_rollout_mode === 'apply' ? 'border-yellow-500/40 bg-yellow-900/20 text-yellow-200' : 'border-blue-500/40 bg-blue-900/20 text-blue-200'"
        >
        ...
        <div class="bg-gray-900 rounded-lg p-4 border border-gray-700">
          <div class="flex items-center justify-between mb-3">
            <h4 class="text-sm font-medium text-gray-200">
              Shadow Promotion Metrics (Read-only)
            </h4>
            <button
          ...
          <div
            v-if="promotionMetricsAvailable"
            class="space-y-3"
          >
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 text-sm">
              <div class="bg-gray-800/70 rounded p-2 border border-gray-700">
                ...
                <p class="text-white font-medium">
                  {{ promotionMetrics.metrics.shadow_sample_count }}
                </p>
              </div>
              <div class="bg-gray-800/70 rounded p-2 border border-gray-700">
                <p class="text-gray-400">
                ...
                <p class="text-white font-medium">
                  {{ formatMetricPercent(promotionMetrics.metrics.correction_delta) }}
                </p>
              </div>
              <div class="bg-gray-800/70 rounded p-2 border border-gray-700">
                <p class="text-gray-400">
                ...
                <p class="text-white font-medium">
                  {{ formatMetricPercent(promotionMetrics.metrics.error_rate_delta) }}
                </p>
              </div>
              <div class="bg-gray-800/70 rounded p-2 border border-gray-700">
                <p class="text-gray-400">
                ...
                <p class="text-white font-medium">
                  {{ formatMs(promotionMetrics.metrics.p95_latency_delta_ms) }}
                </p>
              </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs">
              <div
                ...
                :class="promotionMetrics.metrics.shadow_sample_count >= promotionMetrics.gates.min_samples ? 'bg-green-900/20 border-green-500/40 text-green-300' : 'bg-gray-800 border-gray-700 text-gray-300'"
              >
                Sample gate: {{ promotionMetrics.metrics.shadow_sample_count }} / {{ promotionMetrics.gates.min_samples }}
              </div>
              <div
                class="rounded p-2 border"
                :class="promotionMetrics.metrics.error_rate_delta <= promotionMetrics.gates.max_error_rate_delta ? 'bg-green-900/20 border-green-500/40 text-green-300' : 'bg-gray-800 border-gray-700 text-gray-300'"
              >
                Error gate: ≤ {{ formatMetricPercent(promotionMetrics.gates.max_error_rate_delta) }}
              </div>
              <div
                class="rounded p-2 border"
                :class="promotionMetrics.metrics.p95_latency_delta_ms <= promotionMetrics.gates.max_p95_latency_delta_ms ? 'bg-green-900/20 border-green-500/40 text-green-300' : 'bg-gray-800 border-gray-700 text-gray-300'"
              >
    ...
    <div class="bg-gray-800 border border-gray-700 rounded-lg p-6">
      <div class="flex items-center justify-between mb-4">
        <div>
          ...
          <p class="text-sm text-gray-400">
            Enable embedding cache to reduce duplicate requests
          </p>
        </div>
        <label class="relative inline-flex items-center cursor-pointer">
          <input
      ...
      <div v-if="config.cache_enabled">
        ...
        >
        <p class="mt-1 text-xs text-gray-500">
          How long to cache embeddings (default: 24 hours)
        </p>
      </div>
    </div>
    ...
    <div class="bg-gray-800 border border-gray-700 rounded-lg p-6">
      ...
      <div class="space-y-3">
        ...
        <div class="flex items-center justify-between p-3 bg-gray-700/30 rounded-lg">
          <div>
            <p class="font-medium text-white">
              Log Embedding Content
            </p>
            <p class="text-sm text-gray-400">
              Warning: Significantly increases log size
            </p>
          </div>
          <label class="relative inline-flex items-center cursor-pointer">
            <input
      ...
      <div class="space-y-4">
        <div class="flex items-center justify-between p-4 bg-gray-800/50 rounded-lg">
          <div class="flex-1">
            ...
            <p class="text-sm text-gray-400">
              Remove all generated embeddings. They will need to be regenerated.
            </p>
          </div>
          <button
