    ...
    <template v-else>
        ...
        <div class="space-y-6">
          ...
          <div class="space-y-2">
            ...
            <Slider
              ...
              :max="95"
              :step="5"
              @update:model-value="validateThresholds"
            />
          </div>
          ...
          <div
            v-if="!isValid"
            class="p-3 bg-red-900/30 text-red-400 rounded-lg text-sm border border-red-500/30"
          >
            ...
            <div class="flex items-center gap-3 p-3 bg-green-500/10 border border-green-500/30 rounded">
              ...
              <div class="flex-1">
                <div class="font-medium text-green-400">
                  Auto-Classify ({{ policySettings.autoClassifyThreshold }}% - 100%)
                </div>
                <div class="text-sm text-gray-400">
                  Immediate routing, no user input
                </div>
              </div>
            </div>
            ...
            <div class="flex items-center gap-3 p-3 bg-yellow-500/10 border border-yellow-500/30 rounded">
              ...
              <div class="flex-1">
                <div class="font-medium text-yellow-400">
                  Prompt Confirm ({{ policySettings.promptThreshold }}% - {{ Math.max(policySettings.autoClassifyThreshold - 1, policySettings.promptThreshold) }}%)
                </div>
                <div class="text-sm text-gray-400">
                  Medium-confidence items prompt you for quick Yes/No confirmation before routing.
                </div>
              </div>
            </div>
            ...
            <div class="flex items-center gap-3 p-3 bg-red-500/10 border border-red-500/30 rounded">
              ...
              <div class="flex-1">
                <div class="font-medium text-red-400">
                  Policy Builder (0% - {{ Math.max(policySettings.promptThreshold - 1, 0) }}%)
                </div>
                <div class="text-sm text-gray-400">
                  Detailed signal breakdown with policy creation guidance
                </div>
              </div>
            </div>
          </div>
      ...
      <Card>
        ...
        <div class="space-y-4">
          ...
          <div
            v-if="fallbackIncident"
            class="p-4 bg-red-500/10 border border-red-500/30 rounded-lg space-y-3"
          >
            ...
            <div class="flex flex-wrap gap-2">
              <Button
                size="sm"
                variant="outline"
                @click="copyFallbackIncidentReport"
              >
                📋 Copy Report
              </Button>
              <Button
      ...
      <Card>
        <template #header>
          <h2 class="text-xl font-semibold">
            🔔 Discord Notification - Display Options
          </h2>
        </template>
        ...
        <div class="space-y-6">
          ...
          <div class="space-y-3">
            ...
            <label class="flex items-center gap-2">
              <input
                v-model="discordSettings.includeSignalBreakdown"
                type="checkbox"
            ...
            <label class="flex items-center gap-2">
              <input
                v-model="discordSettings.showSimilarItems"
                type="checkbox"
                class="w-4 h-4 rounded bg-gray-800 border-gray-600 text-blue-500 focus:ring-blue-500 focus:ring-offset-gray-900"
              >
      ...
      <Card>
        <template #header>
          <h2 class="text-xl font-semibold">
            🧠 Auto-Learning - Feedback Thresholds
          </h2>
        </template>
        ...
        <div class="space-y-8">
          ...
          <div class="space-y-4 pt-6 border-t border-gray-700">
            ...
            <select
              v-model="learningSettings.conflictResolutionStrategy"
              class="w-full md:w-96 px-3 py-2 bg-gray-800 border border-gray-700 rounded"
            >
              <option value="block">
                🚫 Block - Don't learn (Conservative)
              </option>
              <option value="escalate">
                📋 Escalate to Admin Review (Recommended)
              </option>
              <option value="auto_resolve">
                ⚡ Auto-Resolve with High Threshold (Aggressive)
              </option>
            </select>

            <div
              v-if="learningSettings.conflictResolutionStrategy === 'auto_resolve'"
              class="space-y-2"
            >
              ...
              >
              <p class="text-xs text-gray-400">
                Confirmations needed to override an exclusion rule
              </p>
            </div>
          </div>
        ...
        >
          <table class="w-full text-sm">
            ...
            <tbody>
              ...
              >
                <td class="py-2">
                  {{ formatSettingKey(audit.settingKey) }}
                </td>
                <td class="py-2 text-gray-400">
                  {{ formatSettingValue(audit.oldValue) }}
                </td>
                <td class="py-2 text-blue-400">
                  {{ formatSettingValue(audit.newValue) }}
                </td>
                <td class="py-2">
                ...
                <td class="py-2 text-gray-400">
                  {{ audit.changeReason || 'Manual update' }}
                </td>
                <td class="py-2">
                  <Button
                    size="sm"
                    variant="danger"
                    :disabled="!audit.canRevert"
                    @click="revertSetting(audit.id)"
                  >
                    Revert
                  </Button>
                </td>
