...
onMounted(async () => {
  try {
    const response = await axios.get('/api/settings/radarr')
    if (response.data) {
      config.value = {
        protocol: response.data.protocol || 'http',
        host: response.data.host || 'localhost',
        port: response.data.port || 7878,
...
const testConnection = async () => {
  ...
  try {
    const response = await axios.post('/api/settings/radarr/test', config.value)
...
const saveSettings = async () => {
  ...
  try {
    await axios.put('/api/settings/radarr', config.value)
    saveStatus.value = { type: 'success', message: 'Radarr settings saved successfully!' }
