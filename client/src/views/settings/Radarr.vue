  ...
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-gray-400 text-sm">Configure your Radarr movie manager connections</p>
      </div>
    ...
    <div v-for="(instance, index) in configs" :key="instance.id" class="space-y-4">
      ...
      <div v-if="isEditing && editingId === instance.id" class="bg-gray-800 border border-gray-700 rounded-lg p-6 space-y-4">
        <div class="flex justify-between items-center">
          <h3 class="text-lg font-medium">Editing: {{ instance.name || `Radarr ${index + 1}` }}</h3>
          <button 
            @click="cancelEdit"
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            <input v-model="editForm.name" type="text" placeholder="Radarr 4K" class="w-full px-4 py-2 bg-gray-900 border border-gray-600 rounded-lg" />
          </div>
          <div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            <input v-model.number="editForm.port" type="number" placeholder="7878" class="w-full px-4 py-2 bg-gray-900 border border-gray-600 rounded-lg" />
          </div>
          <div>
            <label class="block text-sm font-medium mb-2">Base Path</label>
            <input v-model="editForm.base_path" type="text" placeholder="/radarr" class="w-full px-4 py-2 bg-gray-900 border border-gray-600 rounded-lg" />
          </div>
          <div>
            <label class="block text-sm font-medium mb-2">API Key</label>
            <PasswordInput v-model="editForm.api_key" placeholder="Your Radarr API key" />
          </div>
        </div>
    ...
    <div v-if="isAddingNew || configs.length === 0" class="bg-gray-800 border border-gray-700 rounded-lg p-6 space-y-4">
      <div class="flex justify-between items-center">
        <h3 class="text-lg font-medium">{{ configs.length === 0 ? 'Configure Radarr' : 'Add New Radarr Instance' }}</h3>
        <button 
