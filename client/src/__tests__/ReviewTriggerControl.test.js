...
import ReviewTriggerControl from '@/components/policies/ReviewTriggerControl.vue'
...
describe('ReviewTriggerControl.vue', () => {
  it('stages a server-approved review condition without presenting it as a blocker or requiring confirmation', async () => {
    const wrapper = mount(ReviewTriggerControl, {
      props: {
    ...
    expect(wrapper.text()).toContain('does not block automatic application')
    expect(wrapper.text()).not.toContain('This is a blocker')
    expect(wrapper.find('input[type="checkbox"]').exists()).toBe(false)
    expect(wrapper.get('select').attributes('aria-describedby'))
      .toBe('policy-intent-constraint-review_warning-description policy-intent-constraint-controls-status')
    expect(stageButton.attributes('disabled')).toBeDefined()

    await wrapper.get('#policy-intent-constraint-review_warning-value').setValue('evidence_conflicting')
    expect(stageButton.attributes('aria-label')).toBe('Stage review warning')
  ...
  it('fails closed for an invalid decision model instead of exposing a review condition control', () => {
    const wrapper = mount(ReviewTriggerControl, {
      props: {
