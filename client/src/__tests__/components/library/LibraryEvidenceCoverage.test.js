...
describe('LibraryEvidenceCoverage', () => {
  it('loads read-only evidence and keeps unverified cache status explicit', async () => {
    ...
    expect(wrapper.text()).toContain('1 of 2 eligible TMDB-linked movie/TV identities')
    ...
    expect(wrapper.find('[role="status"]').text()).toContain('can still support background learning')
    expect(wrapper.text()).toContain('cache coverage is not measured here')
    expect(wrapper.text()).toContain('cache and retry counts cover TMDB-linked identities only')
    expect(wrapper.text()).not.toContain('not yet in the description-vector corpus')
