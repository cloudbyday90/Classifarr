...
function buildLifecycle({
  ...
  const isCompatibility = statusId === 'existing_compatibility_policy'
  ...
  return {
    ...
    action: isEligible
      ...
      : isRecovery
        ...
        : isCompatibility
          ? { id: 'review_reconciliation', available: true }
    ...
    proposal: isEligible
      ...
      : isRecovery
        ...
        : isExisting
          ? { available: false, reasonId: statusId }
...
describe('PolicyList.vue', () => {
  ...
  it('routes an existing compatibility policy directly to its reconciliation maintenance record', async () => {
    state.getPolicyAuthoringLifecycle.mockImplementation(libraryId => Promise.resolve(
      libraryId === 7
        ? buildLifecycle({ libraryId: 7, libraryName: 'Movies' })
        : buildLifecycle({
    ...
    await wrapper.find('#policy-authoring-lifecycle-maintenance-action-8').trigger('click')
