...
import AvoidControl from '@/components/policies/AvoidControl.vue'
...
describe('AvoidControl.vue', () => {
  it('requires confirmation before emitting the established advisory avoid plan', async () => {
    const wrapper = mount(AvoidControl, {
      props: {
    ...
    expect(wrapper.get('select').attributes('aria-describedby'))
      .toBe('policy-intent-constraint-avoid-description policy-intent-constraint-controls-status')
    expect(stageButton.attributes('disabled')).toBeDefined()

    await wrapper.get('#policy-intent-constraint-avoid-value').setValue('R')
    expect(stageButton.attributes('aria-label')).toContain('confirm this explicit operator choice first')
    await wrapper.get('#policy-intent-constraint-avoid-confirmation').setValue(true)
    ...
    expect(wrapper.get('#policy-intent-constraint-avoid-value').element.value).toBe('')
    expect(wrapper.get('#policy-intent-constraint-avoid-confirmation').element.checked).toBe(false)
  })

  it('fails closed for an invalid decision model instead of exposing an avoid control', () => {
    const wrapper = mount(AvoidControl, {
      props: {
