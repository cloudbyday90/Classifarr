...
<template>
  <div class="space-y-4">
    ...
    >
      <div>
        ...
        <p class="text-sm text-gray-400">
          Map Plex libraries to {{ arrType === 'radarr' ? 'Radarr' : 'Sonarr' }} root folders
        </p>
      </div>
      <button 
    ...
    <div
      v-if="!mediaServerId"
      class="p-4 bg-yellow-900/20 border border-yellow-600/30 rounded-lg"
    >
    ...
    <template v-else>
        ...
        <table class="w-full">
          ...
          <tbody>
            ...
            >
              ...
              <td class="p-3">
                <code
                  v-if="mapping.classifarr_path_prefix"
                  class="text-xs bg-gray-900 px-2 py-1 rounded-sm"
                >{{ mapping.classifarr_path_prefix }}</code>
                <span
                  v-else
              ...
              <td
                v-if="!readonly"
                class="p-3 text-right flex gap-2 justify-end"
              >
                <button 
      ...
      <div
        v-if="!loading && mappings.length === 0 && unmappedLibraries.length === 0"
        class="text-center py-6 text-gray-400 border border-gray-700 border-dashed rounded-lg"
      >
        <p v-if="readonly">
          No library mappings configured for this {{ arrType === 'radarr' ? 'Radarr' : 'Sonarr' }} instance
        </p>
        <p v-else>
          No {{ arrType === 'radarr' ? 'movie' : 'TV' }} libraries found on the associated media server
        </p>
      </div>
    </template>
    ...
    <details
      v-if="!readonly && mediaServerId"
      class="bg-gray-800/30 border border-gray-700 rounded-lg"
    >
      ...
      <div class="px-4 pb-4 text-sm text-gray-400 space-y-3">
        ...
        <div class="bg-gray-900/50 p-3 rounded-sm">
          <h5 class="font-medium text-gray-300 mb-2">
            Docker Path Mapping
          </h5>
          <p class="text-xs">
        ...
        <div class="p-4 space-y-4">
          ...
          <div>
            ...
            >
              ...
              <option
                v-for="folder in availableRootFolders"
                :key="folder.id"
                :value="folder.id + ':' + folder.path"
              >
                {{ folder.path }}
              </option>
            </select>
          ...
          <div v-if="mappingForm.root_folder">
            ...
            <p class="text-xs text-gray-500 mt-1">
              Path as seen inside Classifarr container
            </p>
          </div>
    ...
    >
      <div class="bg-gray-800 rounded-lg border border-gray-700 max-w-lg w-full mx-4 max-h-[80vh] flex flex-col">
        ...
        <div class="flex-1 overflow-y-auto p-4">
          ...
          <div
            v-else-if="browserFolders.length === 0"
...
const closeMappingModal = () => {
  ...
  mappingForm.value = { library_id: null, library_name: '', root_folder: '', classifarr_path: '' }
}
