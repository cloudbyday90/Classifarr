...
<template>
  <section
    ...
    :class="{ 'has-errors': failedQueueTasks.length > 0 }"
  >
    <div
      class="secondary-section-header"
      @click="$emit('toggle-section', 'errors')"
    >
      <h2 class="secondary-section-title">
        Errors
        <span
          v-if="failedQueueTasks.length"
          class="secondary-section-count"
        >({{ failedQueueTasks.length }})</span>
      </h2>
    ...
    >
      ...
      >
        <article
          v-for="task in failedQueueTasks"
          :key="task.id"
          class="error-item"
        >
          ...
          <div class="error-actions">
            <Button
              variant="warning"
              size="sm"
              @click="$emit('retry-failed-task', task.id)"
            >
              Retry
            </Button>
            <Button
              variant="ghost"
              size="sm"
              @click="$emit('dismiss-failed-task', task.id)"
            >
              Dismiss
            </Button>
          </div>
  ...
  <section
    v-if="showEnrichmentSection"
    id="enrichment"
    class="secondary-section"
  >
    <div
      class="secondary-section-header"
      @click="$emit('toggle-section', 'enrichment')"
    >
      <h2 class="secondary-section-title">
        Enrichment
    ...
    <div
      v-if="expandedSections.enrichment"
      class="secondary-section-content"
    >
      <div class="enrichment-progress">
        ...
        <div
          class="enrichment-state-grid"
          aria-label="Enrichment state summary"
        >
          ...
          <div class="enrichment-state-card enrichment-state-card-info">
            ...
            <Badge variant="info">
              {{ formatNumber(enrichmentProcessingItems) }}
            </Badge>
          </div>
          <div class="enrichment-state-card enrichment-state-card-warning">
            <span class="enrichment-state-label">Pending</span>
            <Badge variant="warning">
              {{ formatNumber(enrichmentPendingItems) }}
            </Badge>
          </div>
          <div
            class="enrichment-state-card enrichment-state-card-warning"
            :class="{ 'enrichment-state-card-muted': enrichmentDeferredItems === 0 }"
          >
            <span class="enrichment-state-label">Deferred</span>
            <Badge :variant="enrichmentDeferredItems > 0 ? 'warning' : 'default'">
              {{ formatNumber(enrichmentDeferredItems) }}
            </Badge>
          </div>
          <div
            class="enrichment-state-card enrichment-state-card-error"
            :class="{ 'enrichment-state-card-muted': enrichmentFailedItems === 0 }"
          >
        ...
        <p class="enrichment-stats">
          ...
          • OMDb: {{ formatNumber(enrichmentOmdb) }}<span
            v-if="enrichmentOmdbPending > 0"
  ...
  >
    <div
      class="secondary-section-header"
      @click="$emit('toggle-section', 'recent')"
    >
      <h2 class="secondary-section-title">
        Recently Completed
      </h2>
      <div class="secondary-section-actions">
        <router-link
          :to="{ path: '/history', query: { source: 'command-center' } }"
          class="secondary-section-link"
          @click.stop
        >
          View History →
        </router-link>
    ...
    <div
      v-if="expandedSections.recent"
      class="secondary-section-content"
    >
      <div
        v-if="recentlyCompletedItems.length"
        class="recent-list"
      >
        <div
          v-for="item in recentlyCompletedItems"
          :key="`recent-${item.id}`"
          class="recent-item"
        >
  ...
  >
    <div
      class="secondary-section-header"
      @click="$emit('toggle-section', 'libraries')"
    >
      <h2 class="secondary-section-title">
        Libraries
      </h2>
    ...
    >
      ...
      >
        ...
        <Button
          variant="warning"
          size="sm"
          @click="$emit('open-media-server-settings')"
        >
          Configure Media Server
        </Button>
      </div>
      <div
        v-if="activeLibrariesSummary.length"
        class="libraries-list"
      >
        <div
          v-for="library in activeLibrariesSummary"
          :key="`library-summary-${library.id}`"
          class="library-item"
        >
      ...
      <p
        v-else-if="!showConfigureMediaServerCta"
        class="secondary-empty"
      >
        No active libraries found.
      </p>
    </div>
  </section>
  ...
  <section
    id="today"
    class="secondary-section secondary-section-last"
  >
    <div
      class="secondary-section-header"
      @click="$emit('toggle-section', 'today')"
    >
      <h2 class="secondary-section-title">
        Today's Summary
      </h2>
