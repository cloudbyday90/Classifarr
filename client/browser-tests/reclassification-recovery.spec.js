...
for (const initialStatus of ['paused', 'executing']) {
  test(`history and batch recover while preserving ${initialStatus} intent`, async ({ page }, testInfo) => {
    let recovered = false
    const writes = []
    ...
    const batch = () => ({ id: 1, status: initialStatus === 'executing' && recovered ? 'completed' : initialStatus, error_message: 'Original dependency unavailable',
      items: [{ ...item, status: recovered ? 'completed' : initialStatus === 'paused' ? 'failed' : 'executing', move_recovery: recovery() }],
    ...
    await page.route(url => url.pathname.startsWith('/api/'), async route => {
      ...
      await route.fulfill({ status: path.endsWith('/execute') ? 202 : 200, contentType: 'application/json', body: JSON.stringify(data) })
    })
    ...
    if (initialStatus === 'paused') await expect(page.getByText('Execution Paused')).toBeVisible()
    else await expect(page.getByText(/Closing this window does not pause it/)).toBeVisible()
    recovered = true
    ...
    if (initialStatus === 'paused') await expect(dialog.getByText(/there is nothing left to resume/)).toBeVisible()
    else await expect(dialog.getByText('Batch Complete', { exact: true })).toBeVisible()
