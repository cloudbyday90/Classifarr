...
test('automatically presents scoped statistics and evidence with keyboard and mobile access', async ({ page }, testInfo) => {
  ...
  await expect(observationTypes.getByRole('rowheader')).toHaveText(['Observed movies', 'All recorded libraries'])
  await expect(observationTypes.getByRole('row').last().getByRole('cell')).toHaveText(['10', '0', '0', '9', '1'])
  ...
  await expect(section.getByRole('region', { name: 'History evidence table', exact: true })
    .getByRole('columnheader', { name: 'Imported membership', exact: true })).toBeVisible()
  ...
  await section.locator('.original-observation-types').screenshot({ path: testInfo.outputPath('observation-types-desktop.png'), animations: 'disabled' })
  ...
  await page.keyboard.press('ArrowRight')
  await expect.poll(() => observationTypes.evaluate(element => element.scrollLeft)).toBeGreaterThan(0)
  expect(await observationTypes.getByRole('columnheader').first().evaluate(textContrast)).toBeGreaterThanOrEqual(4.5)
  await page.setViewportSize({ width: 320, height: 2200 })
  await observationTypes.evaluate(element => { element.scrollLeft = 0 })
  await section.locator('.original-observation-types').screenshot({ path: testInfo.outputPath('observation-types-mobile.png'), animations: 'disabled' })
  await page.setViewportSize({ width: 320, height: 844 })
