...
// Full headers to insert when a file has none
const HEADERS = {
  js: `/*
 * Classifarr - AI-powered media classification for the *arr ecosystem
...
// Matches any existing copyright line so we can update year + owner in one pass
const COPYRIGHT_RE = /Copyright \(C\) (\d{4}|\d{4}-\d{4}) (cloudbyday90|Classifarr Contributors)/g;

function processFile(filePath) {
  let content = fs.readFileSync(filePath, 'utf8');

  // --- Case 1: file has no copyright notice at all → insert full header ---
  ...
  // --- Case 2: header exists → update year range and owner if stale ---
