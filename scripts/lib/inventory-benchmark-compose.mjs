...
export function inventoryBenchmarkComposePlan(container, argv, runId, freeMemory) {
  const { values } = parseArgs({ args: argv, options: Object.fromEntries([
    ...['seed', 'size', 'folds', 'generate-cases', 'context', 'max-minutes', 'exclude-prior-sizes', 'exclude-prior-size']
      .map(name => [name, { type: 'string' }]),
    ...['leader-grounded', 'leader-semantic', 'leader-challenge'].map(name => [name, { type: 'boolean' }]),
  ]) });
  const modes = ['leader-grounded', 'leader-semantic', 'leader-challenge'].filter(name => values[name]);
  ...
  if (modes.length !== 1 || !options.folds || options.generateCases > 32 ||
      (modes[0] === 'leader-challenge' && options.generateCases !== 0) ||
