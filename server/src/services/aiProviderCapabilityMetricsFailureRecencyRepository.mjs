...
export const LOAD_AI_PROVIDER_CAPABILITY_METRICS_FAILURE_RECENCY_SQL = `
...
function isUtcDayBoundary(value) {
...
function assertRecencyWindow(window = {}) {
  if (!Array.isArray(window.periods)
    || window.periods.length !== AI_PROVIDER_CAPABILITY_METRICS_HEALTH_TREND_PERIOD_IDS.length) {
    throw new TypeError('A fixed three-window capability-metrics failure-recency range is required.');
  }
  ...
  for (const id of AI_PROVIDER_CAPABILITY_METRICS_HEALTH_TREND_PERIOD_IDS) {
    ...
    if (!(period?.start instanceof Date) || Number.isNaN(period.start.getTime())
      ...
      || !isUtcDayBoundary(period.start)
      || !isUtcDayBoundary(period.end)
...
/**
 * Reads only server-owned, adjacent completed UTC-day warning counts. It
 * cannot return raw metadata, error text, provider/model values, a timestamp,
 * or caller-selected categories, periods, and diagnostic dimensions.
 */
export async function loadAiProviderCapabilityMetricsFailureRecency(database, window = {}) {
  if (!database || typeof database.query !== 'function') {
    throw new TypeError('Capability-metrics failure recency requires a query-capable database.');
  }
  assertRecencyWindow(window);

  const byId = new Map(window.periods.map((period) => [period.id, period]));
  const result = await database.query(LOAD_AI_PROVIDER_CAPABILITY_METRICS_FAILURE_RECENCY_SQL, [
