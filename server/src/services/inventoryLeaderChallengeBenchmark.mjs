...
export async function runInventoryLeaderChallengeBenchmark(settings, {
  ...
  try {
    return await runtime.withDiscoveryAdmission(async (signal, checkpoint) => {
      ...
      const prepared = await prepareLeaderChallengeEvidence(source, representation.dimensions, options, { signal, checkpoint });
      const rows = [];
      for (const doc of prepared.sample) {
        ...
        rows.push({ assessment, observed: doc.libraryIds, mediaType: doc.type, retainedHistory: source.trainingExclusions.has(doc.key) });
      ...
      return { protocol: 'inventory_leader_challenge_v1', status: changedComponents.length ? 'invalidated'
    ...
    if (error instanceof DiscoveryDeferredError) return { protocol: 'inventory_leader_challenge_v1', status: 'deferred',
