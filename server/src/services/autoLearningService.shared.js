...
const DEFAULT_THRESHOLDS = {
    ...
    studioLearnThreshold: 2,          // Need 2+ confirmations before learning studio
    minConfidenceRate: 0.75,          // Require 75% confirmation rate (confirms / (confirms + rejects))
    maxLearnsPerUserPerDay: 50,       // Maximum learns per user per day
    maxLearnsPerLibraryPerHour: 20,   // Maximum learns per library per hour
...
class AutoLearningService {
    ...
    async learnFromFeedback(feedbackData) {
        try {
            ...
            // Learn keywords (limit to top 5 most relevant)
    ...
    /**
     * Learn genre preference for a library
     * @param {number} libraryId - Library ID
     * @param {string} genre - Genre name
     * @param {object} feedback - Feedback context
     * @returns {Promise<object>} Learning result
     */
    async learnGenrePreference(libraryId, genre, feedback) {
        try {
            // Calculate net confidence for this genre
            ...
            // Check if meets threshold and confidence rate
    ...
    /**
     * Learn keyword preference for a library
     * @param {number} libraryId - Library ID
    ...
    async learnKeywordPreference(libraryId, keyword, feedback) {
        try {
            // Calculate net confidence for this keyword
    ...
    /**
     * Learn studio preference for a library
     * @param {number} libraryId - Library ID
    ...
    async learnStudioPreference(libraryId, studio, feedback) {
        try {
            // Calculate net confidence for this studio
    ...
    /**
     * Add genre to library's prefer list
     */
    async addGenreToPrefer(libraryId, genre, confirmCount, userId) {
        ...
        try {
            ...
            // Add genre to prefer list in policy_presets
            await client.query(`
            ...
            // Log the learning in auto_learned_preferences
            await client.query(`
    ...
    /**
     * Add keyword to library's prefer list
     */
    ...
    /**
     * Add studio to library's prefer list
     */
    ...
    /**
     * Calculate net confidence for a signal
     * @param {number} libraryId - Library ID
     * @param {string} value - Signal value (genre, keyword, studio)
    ...
    /**
     * Detect if there's a conflict with existing signals
     * @param {number} libraryId - Library ID
    ...
    async detectIntraLibraryConflict(libraryId, value, preferenceType) {
        try {
            ...
            // On error, be conservative and assume conflict
            return { conflict: true, type: 'error' };
        }
    }

    /**
     * Check if learning can be applied (rate limiting)
     * @param {string} userId - User ID (Discord user ID)
     * @param {number} libraryId - Library ID
    ...
    async canApplyLearning(userId, libraryId) {
        try {
            ...
            // Check library rate limit (per hour)
    ...
    /**
     * Get auto-learned preferences for a library (for admin review)
     */
