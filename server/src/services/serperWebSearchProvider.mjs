...
import { serperProviderClient as defaultSerperClient } from './serperProviderClient.mjs';
import {
...
export function createSerperWebSearchProvider({ serperClient = defaultSerperClient } = {}) {
  return {
    contractVersion: 1,
    providerKey: 'serper',
    ...
    async testConnection(config = {}) {
      return serperClient.testConnection(config.apiKey, getProviderConfig(config));
    },
    ...
    async search(request, config = {}) {
      ...
      try {
        const rawResponse = await serperClient.search(validatedRequest.query, {
          ...
          hl: getConfigValue(config, 'hl', null),
        });
...
export const serperWebSearchProvider = createSerperWebSearchProvider();
