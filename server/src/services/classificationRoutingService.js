...
/**
 * classificationRoutingService — Phase 3 extraction from classification.js
 ...
 *   - resolveRoutingConfig(library)
 *   - routeToArr(metadata, library)
...
/**
 * Parse and normalise an arr settings value. Strings are JSON-decoded; nullish
 * or invalid values return an empty object.
 *
 * @param {*} settings
 * @returns {object}
 */
...
/**
 * Coerce a quality-profile ID to a positive integer, or null if invalid.
 *
 * @param {*} value
 * @returns {number|null}
 */
...
/**
 * Return true when the settings object (or JSON string) is empty / missing.
 *
 * @param {*} settings
 * @returns {boolean}
 */
...
// ---------------------------------------------------------------------------
// Default profile / root-folder resolution
// ---------------------------------------------------------------------------

/**
 * Look up the first quality-profile ID for an arr instance, checking the
 * local cache table before hitting the arr API.
 *
...
/**
 * Look up the first root-folder path for an arr instance, checking the local
 * cache table before hitting the arr API.
 *
 * @param {'radarr'|'sonarr'} arrType
 * @param {string} baseUrl
 * @param {string} apiKey
...
/**
 * Resolve a library object into a fully-populated routing config by merging
 * any matching library_arr_mappings row.
 *
...
/**
 * Route a classified item to its target Radarr or Sonarr instance.
 *
 * Returns a routing-result shape:
 *   { attempted, routed, arrType, reason, error }
 *
...
// ---------------------------------------------------------------------------
// Series type suggestion
// ---------------------------------------------------------------------------

/**
 * Suggest a Sonarr series type based on metadata and applied classification
 * labels.
 *
 * @param {object} metadata
 * @param {string[]} appliedLabels
 * @returns {'anime'|'daily'|'standard'}
 */
