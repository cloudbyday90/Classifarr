...
export const CAPTURE_SOURCE_OBSERVATIONS = `WITH incoming AS MATERIALIZED (
    SELECT * FROM jsonb_to_recordset($4::jsonb) AS i(external_id text, title text, year integer,
      media_type text, identity_issue text, provider_fields text[], source_digest text)
    ...
    SELECT $1,$2,external_id,title,year,media_type,identity_issue,provider_fields,$3,source_digest FROM admitted
    ON CONFLICT (library_id, media_server_id, external_id) DO UPDATE SET title=EXCLUDED.title,
      ...
      provider_fields=EXCLUDED.provider_fields, generation=EXCLUDED.generation,
