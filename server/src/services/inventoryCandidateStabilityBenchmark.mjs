...
const summarize = ({ pairedWithComplete, ...metrics }) => ({ ...metrics, pairedWithAligned: pairedWithComplete });
...
export async function runInventoryCandidateStabilityBenchmark(snapshot, dimensions, rawOptions, { signal, onProgress,
  ...
  const arms = ['aligned', 'independent'].map(name => ({ name, ...createCoverageMetrics(),
  ...
  for (let fold = 0; fold < options.folds; fold++) {
    ...
    for (const doc of sample.filter(row => folds.foldByHash.get(row.hash) === fold)) {
      ...
      const baseline = withPlacement(decisions.aligned, candidates, doc);
    ...
    onProgress?.({ phase: 'candidate_stability', fold: fold + 1, evaluated: arms[0].evaluated });
  }
  abort.throwIfAborted();
  return { protocol: 'inventory_candidate_stability_v1', status: 'complete', calls: 0, livePromotionAllowed: false,
    ...
    arms: arms.map(arm => ({ ...summarize(arm), mediaTypes: arm.mediaTypes.map(summarize),
      libraries: arm.libraries.map(summarize), supportRanges: arm.supportRanges.map(summarize) })) };
}
