...
export class QueueMaintenanceService {
    ...
    async runScheduledTaskQueueCleanup() {
        ...
        try {
            ...
            if (totalDeleted > 0) {
                this.logger.info('Task queue cleanup complete', { deleted: totalDeleted, retentionDays });
                try {
                    await this.db.query('VACUUM ANALYZE task_queue');
                ...
                this.logger.debug('Task queue cleanup: no rows to delete', { retentionDays, maxTotalRows: MAX_TOTAL_ROWS });
            }
        } catch (error) {
