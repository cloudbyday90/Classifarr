...
import { INVENTORY_REPRESENTATIVE_STABILITY_VERSION } from './inventoryRepresentativeStability.mjs';
import { rankStableInventoryRepresentativeEvidence } from './inventoryRepresentativeStabilityRanking.mjs';
...
export async function runInventoryEvidenceRerankerComparison(snapshot, dimensions, settings,
  { signal, onProgress = () => {}, neighborhoodProfiles = false, representativeGroups = false, representativeStability = false } = {}) {
  ...
  if (representativeStability && !representativeGroups) throw new Error('inventory_representative_stability_requires_groups');
  ...
  const representativeIndex = representativeGroups ? createInventoryRepresentativeIndex(snapshot, dimensions, options.folds, { stability: representativeStability }) : null;
  ...
  const representativeVersion = representativeStability ? INVENTORY_REPRESENTATIVE_STABILITY_VERSION : INVENTORY_REPRESENTATIVE_VERSION;
  const representatives = { version: representativeVersion, statuses: {}, folds: [], elapsedFitMs: 0 };
  const legacyResults = [], selectedResults = [];
  for (let fold = 0; fold < options.folds; fold++) {
    ...
    for (const mediaType of ['movie', 'tv']) {
      ...
      for (const row of outer.filter(row => row.entry.mediaType === mediaType && row.candidates.length)) {
        ...
        const local = model ? { ...(representativeStability ? rankStableInventoryRepresentativeEvidence : rankInventoryRepresentativeEvidence)(model, row), support: [] } : neighborhoodProfiles ? rankInventoryNeighborhoodEvidence(index, row,
        ...
        if (representativeStability) {
          for (const [target, ranking] of [[legacyResults, local.legacyRanking], [selectedResults, local.selectedRanking]]) {
            target.push({ before: row.observedLibraryIds.includes(before), after: row.observedLibraryIds.includes(ranking[0]), changed: before !== ranking[0] });
          }
        }
      }
    }
  ...
  await setImmediate();
  abort.throwIfAborted();
  return { version: 1, protocol: representativeGroups ? representativeVersion : neighborhoodProfiles ? INVENTORY_NEIGHBORHOOD_PROFILE_VERSION : 'inventory_evidence_reranker_v1',
    ...(representativeGroups ? { representatives, ...(representativeStability ? { legacyComparison: summarize(legacyResults), selectedComparison: summarize(selectedResults) } : {}) } : {}),
