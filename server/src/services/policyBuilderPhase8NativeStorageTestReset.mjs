const PHASE8R_NATIVE_STORAGE_TEST_RESET_VERSION = 'phase8r.native_storage_test_reset.v1';

const PHASE8R_NATIVE_STORAGE_TEST_COVERAGE_IDS = Object.freeze({
...
const PHASE8R_NATIVE_STORAGE_TEST_LEGACY_SCOPE_IDS = Object.freeze({
...
const PHASE8R_NATIVE_STORAGE_TEST_STATUS_IDS = Object.freeze({
...
const PHASE8R_NATIVE_STORAGE_TEST_RISK_IDS = Object.freeze({
...
const PHASE8R_NATIVE_STORAGE_TEST_REASON_IDS = Object.freeze({
...
const REQUIRED_COVERAGE = Object.freeze([
  {
    coverageId: PHASE8R_NATIVE_STORAGE_TEST_COVERAGE_IDS.NATIVE_SCHEMA_SQL_MIGRATION_TESTS,
    label: 'Native schema SQL migration tests',
    requiredBeforeNativeDefault: true,
  },
  {
    coverageId: PHASE8R_NATIVE_STORAGE_TEST_COVERAGE_IDS.NATIVE_SCHEMA_CONTRACT_TESTS,
    label: 'Native schema contract tests',
    requiredBeforeNativeDefault: true,
  },
  {
    coverageId: PHASE8R_NATIVE_STORAGE_TEST_COVERAGE_IDS.DRY_RUN_CANDIDATE_REPORT_TESTS,
    label: 'Dry-run migration candidate report tests',
    requiredBeforeNativeDefault: true,
  },
  {
    coverageId: PHASE8R_NATIVE_STORAGE_TEST_COVERAGE_IDS.EXPLICIT_CONVERSION_TESTS,
    label: 'Explicit conversion workflow tests',
    requiredBeforeNativeDefault: true,
  },
  {
    coverageId: PHASE8R_NATIVE_STORAGE_TEST_COVERAGE_IDS.NATIVE_RUNTIME_READ_PATH_TESTS,
    label: 'Native runtime read-path tests',
    requiredBeforeNativeDefault: true,
  },
  {
    coverageId: PHASE8R_NATIVE_STORAGE_TEST_COVERAGE_IDS.ROLLBACK_REVERSION_TESTS,
    label: 'Rollback and reversion tests',
    requiredBeforeNativeDefault: true,
  },
  {
    coverageId: PHASE8R_NATIVE_STORAGE_TEST_COVERAGE_IDS.LEGACY_WRITE_BLOCKING_TESTS,
    label: 'Legacy write-blocking tests for converted policies',
    requiredBeforeNativeDefault: true,
  },
  {
    coverageId: PHASE8R_NATIVE_STORAGE_TEST_COVERAGE_IDS.BACKUP_RESTORE_COVERAGE_TESTS,
    label: 'Backup/restore native coverage tests',
    requiredBeforeNativeDefault: true,
  },
  {
    coverageId: PHASE8R_NATIVE_STORAGE_TEST_COVERAGE_IDS.DELETION_GATE_TESTS,
...
const CURRENT_PHASE8R_TEST_RECORDS = Object.freeze([
  ...
  },
  {
    ...
    coverageIds: [PHASE8R_NATIVE_STORAGE_TEST_COVERAGE_IDS.DRY_RUN_CANDIDATE_REPORT_TESTS],
    finalNativeStorageContract: true,
  },
  {
    path: 'server/src/__tests__/services/policyBuilderPhase8ExplicitConversionWorkflow.test.mjs',
    coverageIds: [PHASE8R_NATIVE_STORAGE_TEST_COVERAGE_IDS.EXPLICIT_CONVERSION_TESTS],
    finalNativeStorageContract: true,
  },
  {
    path: 'server/src/__tests__/services/policyBuilderPhase8NativeRuntimeReadPath.test.mjs',
    coverageIds: [PHASE8R_NATIVE_STORAGE_TEST_COVERAGE_IDS.NATIVE_RUNTIME_READ_PATH_TESTS],
    finalNativeStorageContract: true,
  },
  {
    path: 'server/src/__tests__/services/policyBuilderPhase8RollbackSnapshotWindow.test.mjs',
    coverageIds: [PHASE8R_NATIVE_STORAGE_TEST_COVERAGE_IDS.ROLLBACK_REVERSION_TESTS],
    finalNativeStorageContract: true,
  ...
  },
  {
    ...
    coverageIds: [PHASE8R_NATIVE_STORAGE_TEST_COVERAGE_IDS.LEGACY_WRITE_BLOCKING_TESTS],
    finalNativeStorageContract: true,
  },
  {
  ...
  },
  {
    ...
    coverageIds: [PHASE8R_NATIVE_STORAGE_TEST_COVERAGE_IDS.DELETION_GATE_TESTS],
    finalNativeStorageContract: true,
  },
  {
...
function normalizeTestRecord(record = {}) {
  return {
    ...
    deleteAfterPhase8rGates: normalizeBoolean(record.deleteAfterPhase8rGates),
...
function isAllowedLegacyScope(legacyScopeId) {
  return Object.values(PHASE8R_NATIVE_STORAGE_TEST_LEGACY_SCOPE_IDS).includes(legacyScopeId);
}
...
function getUnscopedDiagnosticTestPaths(testRecords = []) {
  return asArray(testRecords)
    .filter(record => (
      ...
      record.deleteAfterPhase8rGates !== true
    ))
    .map(record => record.path);
}
...
function determineStatus({
  ...
  if (missingCoverageIds.length > 0) {
    return PHASE8R_NATIVE_STORAGE_TEST_STATUS_IDS.BLOCKED_BY_MISSING_NATIVE_COVERAGE;
  }

  if (unscopedLegacyTestPaths.length > 0) {
    return PHASE8R_NATIVE_STORAGE_TEST_STATUS_IDS.BLOCKED_BY_LEGACY_SCOPE;
  }
  ...
  if (
    ...
    return PHASE8R_NATIVE_STORAGE_TEST_STATUS_IDS.BLOCKED_BY_DELETION_GATE_SCOPE;
  }

  return PHASE8R_NATIVE_STORAGE_TEST_STATUS_IDS.READY_FOR_NATIVE_STORAGE_TEST_RESET;
}
...
function buildBlockers({
  ...
  missingCoverageIds.forEach(coverageId => {
    blockers.push({
      ...
      coverageId,
      message: 'Required Phase 8R native-storage test coverage is missing.',
    });
  });
  ...
  unscopedDiagnosticTestPaths.forEach(path => {
    blockers.push({
      ...
      path,
      message: 'Abandoned diagnostic UI tests must be marked for deletion after Phase 8R gates pass.',
    });
  });
...
function buildPolicyBuilderPhase8NativeStorageTestReset({
  testRecords = CURRENT_PHASE8R_TEST_RECORDS,
  ...
  const plan = {
    contractVersion: PHASE8R_NATIVE_STORAGE_TEST_RESET_VERSION,
    statusId,
    resetReady:
    ...
    reasons: [
      buildReason(
        PHASE8R_NATIVE_STORAGE_TEST_REASON_IDS.COVERAGE_ALLOW_LIST_DEFINED,
        'Native storage coverage is allow-listed from the Phase 8R roadmap.'
      ),
      buildReason(
        PHASE8R_NATIVE_STORAGE_TEST_REASON_IDS.CURRENT_TESTS_INVENTORIED,
        'Current Phase 8R contract and compatibility tests are inventoried before reset.'
      ),
      buildReason(
        PHASE8R_NATIVE_STORAGE_TEST_REASON_IDS.NATIVE_SQL_MIGRATION_GAP_EXPLICIT,
        'Native SQL migration coverage is tracked explicitly instead of inferred from schema-contract tests.'
      ),
      buildReason(
        PHASE8R_NATIVE_STORAGE_TEST_REASON_IDS.LEGACY_TEST_SCOPE_REQUIRED,
        'Legacy preservation tests must be scoped to unconverted policies, rollback snapshots, or maintainer fixtures.'
      ),
      buildReason(
        PHASE8R_NATIVE_STORAGE_TEST_REASON_IDS.DIAGNOSTIC_TESTS_DELETION_SCOPED,
        'Tests for abandoned diagnostic UI must remain deletion-scoped until Phase 8R gates permit removal.'
      ),
      buildReason(
        PHASE8R_NATIVE_STORAGE_TEST_REASON_IDS.SIDE_EFFECTS_DISABLED,
  ...
  return {
    ...plan,
    validation: validatePolicyBuilderPhase8NativeStorageTestReset(plan),
  };
}

function validatePolicyBuilderPhase8NativeStorageTestReset(plan = {}) {
  const issues = [];
    ...
    if (!coverageIds.has(requirement.coverageId) || coverage?.provided !== true) {
      issues.push({
        riskId: requirement.coverageId ===
          ...
          ? PHASE8R_NATIVE_STORAGE_TEST_RISK_IDS.NATIVE_SQL_MIGRATION_COVERAGE_MISSING
          : PHASE8R_NATIVE_STORAGE_TEST_RISK_IDS.MISSING_REQUIRED_COVERAGE,
  ...
  getUnscopedLegacyTestPaths(testRecords).forEach(path => {
    issues.push({
      riskId: PHASE8R_NATIVE_STORAGE_TEST_RISK_IDS.LEGACY_PRESERVATION_UNSCOPED,
      path,
  ...
  getUnscopedDiagnosticTestPaths(testRecords).forEach(path => {
    issues.push({
      riskId: PHASE8R_NATIVE_STORAGE_TEST_RISK_IDS.DIAGNOSTIC_UI_TEST_NOT_DELETION_SCOPED,
      path,
      message: 'Abandoned diagnostic UI test is not marked for deletion after Phase 8R gates pass.',
    });
  });
  ...
  getDiagnosticTestsPresentAfterDeletionGates(
    ...
    issues.push({
      riskId: PHASE8R_NATIVE_STORAGE_TEST_RISK_IDS.DELETION_GATES_PASSED_WITH_DIAGNOSTIC_TESTS,
      path,
  ...
  getAbandonedDiagnosticsMarkedFinal(testRecords).forEach(path => {
    issues.push({
      riskId: PHASE8R_NATIVE_STORAGE_TEST_RISK_IDS.ABANDONED_DIAGNOSTIC_MARKED_FINAL,
      path,
  ...
  if (hasSideEffects(plan.sideEffects)) {
    issues.push({
      riskId: PHASE8R_NATIVE_STORAGE_TEST_RISK_IDS.SIDE_EFFECT_PERFORMED,
      message: 'Phase 8R.9 reset planning must not delete, rewrite, generate, or mutate tests or schemas.',
    });
  }

  if (asArray(plan.reasons).length === 0) {
    issues.push({
      riskId: PHASE8R_NATIVE_STORAGE_TEST_RISK_IDS.MISSING_REASON,
...
function buildPolicyBuilderPhase8NativeStorageTestResetAudit(
  plan = buildPolicyBuilderPhase8NativeStorageTestReset()
) {
  const validation = validatePolicyBuilderPhase8NativeStorageTestReset(plan);
  ...
  const diagnosticDeletionCandidatePaths = asArray(plan.testRecords)
    .filter(record => record.abandonedDiagnosticUi === true && record.deleteAfterPhase8rGates === true)
    .map(record => record.path);
...
export {
  ...
  PHASE8R_NATIVE_STORAGE_TEST_REASON_IDS,
  PHASE8R_NATIVE_STORAGE_TEST_RESET_VERSION,
