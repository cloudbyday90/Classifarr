...
import { buildGroundedComparisonReport } from './inventoryGroundedComparisonReport.mjs';
...
export function buildLeaderSemanticReport(rows, { eligible, retained, excluded, calls, identity, options, usage, failed }) {
  return { protocol: options.grounded ? 'inventory_leader_grounded_comparison_v1' : 'inventory_leader_semantic_comparison_v1', status: failed ? 'completed_with_errors' : options.generateCases ? 'complete' : 'preflight',
    ...
    ...(options.grounded ? { grounding: buildGroundedComparisonReport(rows) } : {}),
    inference: { ...usage, maximumCalls: Math.min(options.generateCases, rows.length) * (options.grounded ? 5 : 2), model: identity?.model ?? null,
      digest: identity?.digest ?? null, context: options.context, outputLimit: options.grounded ? 4128 : 64, temperature: 0, seed: 42, inputTruncation: 'unknown' },
