...
export function evaluationHistoryCase(key, mediaType, results, label) {
  ...
  return { item: adjudicationDigest(key), mediaType, paired: report.paired === 1,
    ...
    deferralIncreased: report.deferralsIncreased === 1 };
}
...
export function createEvaluationHistory(snapshot, result, cases) {
  ...
  const revision = adjudicationDigest(['evaluation_history.v1', cohortRevision, evidenceRevision, modelRevision]);
  return { version: 'evaluation_history.v1', revision, cohortRevision, evidenceRevision, modelRevision,
...
export function validEvaluationHistory(value, report) {
  if (!exact(value, ['version', 'revision', 'cohortRevision', 'evidenceRevision', 'modelRevision', 'sampled', 'eligible', 'offset', 'cases']) ||
    value.version !== 'evaluation_history.v1' || ![value.revision, value.cohortRevision, value.evidenceRevision, value.modelRevision].every(digest) ||
  ...
  if (!value.cases.every(row => exact(row, ['item', 'mediaType', ...flags]) && digest(row.item) &&
