...
export function evaluationHistoryCase(key, mediaType, results, label) {
  ...
  return { item: adjudicationDigest(key), mediaType, paired: report.paired === 1,
    ...
    deferralIncreased: report.deferralsIncreased === 1, gaps: results.map(evaluationArmGap), pairKind: evaluationPairKind(results) };
}
...
export function createEvaluationHistory(snapshot, result, cases) {
  ...
  const revision = adjudicationDigest(['evaluation_history.v3', cohortRevision, evidenceRevision, modelRevision]);
  return { version: 'evaluation_history.v3', revision, cohortRevision, evidenceRevision, modelRevision,
...
export function validEvaluationHistory(value, report) {
  if (!exact(value, ['version', 'revision', 'cohortRevision', 'evidenceRevision', 'modelRevision', 'sampled', 'eligible', 'offset', 'cases']) ||
    !['evaluation_history.v1', 'evaluation_history.v2', 'evaluation_history.v3'].includes(value.version) ||
  ...
  const gapKeys = value.version !== 'evaluation_history.v1' ? ['gaps'] : [];
  const kindKeys = value.version === 'evaluation_history.v3' ? ['pairKind'] : [];
  if (!value.cases.every(row => exact(row, ['item', 'mediaType', ...flags, ...gapKeys, ...kindKeys]) && digest(row.item) &&
    (!kindKeys.length || (['deterministic', 'mixed', 'ai', 'incomplete'].includes(row.pairKind) && row.paired === (row.pairKind !== 'incomplete'))) &&
    (row.pairKind !== 'deterministic' || (!row.deferralReduced && !row.deferralIncreased)) &&
  ...
  if (gapKeys.length && !['baseline', 'sourceAware'].every((arm, index) => {
    ...
    return replay?.[arm] && completed === (replay[arm].automatic ?? 0) + replay[arm].proposed + replay[arm].abstained && missing === replay[arm].misses &&
      invalid === replay[arm].invalid && reasons.length - completed - missing - invalid === replay[arm].unavailable;
  })) return false;
  if (kindKeys.length && !['deterministic', 'mixed', 'ai'].every(kind =>
    value.cases.filter(row => row.pairKind === kind).length === replay?.[`${kind}Pairs`])) return false;
