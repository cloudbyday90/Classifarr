...
import { collectInventoryCompanyMetadata, projectCompanyTerms } from './inventoryCompanyMetadata.mjs';
import { INVENTORY_COMPANY_PROFILE_VERSION, learnInventoryCompanyProfiles, scoreInventoryCompanyProfile } from './inventoryCompanyProfiles.mjs';
...
export function projectLiveInventoryQueryMetadata(metadata = {}) {
  const companies = projectCompanyTerms(metadata?.production_companies);
  return { ...projectClassificationCandidateMetadata(metadata),
    ...(companies?.length ? { productionCompanies: companies } : {}) };
}
...
export function buildLiveInventoryLearnedProfiles({ rows, corpus, request, modelCache }) {
  ...
  const digest = createHash('sha256').update(JSON.stringify([
    INVENTORY_LEARNED_PROFILE_VERSION, INVENTORY_COMPANY_PROFILE_VERSION, INVENTORY_DESCRIPTION_PROJECTION_VERSION,
  ...
  const fitDigest = createHash('sha256').update(JSON.stringify([
    INVENTORY_LEARNED_PROFILE_VERSION, INVENTORY_COMPANY_PROFILE_VERSION, INVENTORY_DESCRIPTION_PROJECTION_VERSION, libraries,
  ]));
  for (const doc of [...training].sort((a, b) => a.key < b.key ? -1 : a.key > b.key ? 1 : 0)) {
    const value = JSON.stringify([doc.key, doc.hash, [...doc.libraryIds].sort((a, b) => a - b), metadata.get(doc.key), companyMetadata.get(doc.key)]);
  ...
  if (!model) {
    ...
    try { model.companyModel = learnInventoryCompanyProfiles(training, companyMetadata, libraries); }
    catch { /* An optional company feature budget must not disable the established model. */ }
    if (model.summary.trainingDescriptions > 0 || model.companyModel?.summary.trainingDescriptions > 0) {
      modelCache?.set(fitKey, model, estimateInventoryProfileWeight(model));
    }
  }
  return new Map(request.libraryIds.map(libraryId => {
    ...
    const companyFit = scoreInventoryCompanyProfile(model.companyModel, libraryId, request.queryMetadata);
    return [libraryId, { version: INVENTORY_LEARNED_PROFILE_VERSION, snapshotId,
      ...
      // Diagnostic-only channel. The routing/prompt evidence projector deliberately omits it.
      ...(companyFit === null ? {} : { companyProfile: { version: INVENTORY_COMPANY_PROFILE_VERSION,
        relativeFit: Number(companyFit.toFixed(4)), trainingDescriptions: model.companyModel.summary.trainingDescriptions } }),
