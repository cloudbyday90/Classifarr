...
export function summarizeInventoryNeighborFallback(rows) {
  ...
  const familiar = row => row.prepared.matchCalibration?.version === LIBRARY_MATCH_BASELINE_VERSION &&
    row.prepared.matchCalibration.candidates.find(candidate => candidate.libraryId === row.generated.destinationId)?.status === 'familiar';
  const qualified = added.filter(familiar);
  ...
  return { version: INVENTORY_NEIGHBOR_FALLBACK_VERSION, sampled: rows.length,
    ...
    strictPreserved: resolved.length - added.length,
    strictLost: evaluated.filter(row => row.generated.neighborFallback.baseline.wouldResolve && !row.generated.neighborFallback.wouldResolve).length,
