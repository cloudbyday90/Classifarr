...
export function getPatternValue(value) {
...
export function extractPatterns(items) {
    const patterns = [];
    const totalCount = items.length;

    const ratingPattern = extractFieldPattern(
        items,
        'content_rating',
        'equals',
        totalCount
    ...
    const genrePattern = extractArrayPattern(
        items,
        'genres',
        'is_one_of',
        totalCount
    ...
    const collectionPattern = extractArrayPattern(
        items,
        'collections',
        'contains',
        totalCount
    ...
    const tagsPattern = extractArrayPattern(
        items,
        'tags',
        'is_one_of',
        totalCount
...
export function extractFieldPattern(items, field, defaultOperator, totalCount) {
    const valueCounts = {};
    let matchCount = 0;
...
export function extractArrayPattern(items, field, defaultOperator, totalCount) {
    const valueCounts = {};
    ...
    items.forEach(item => {
        ...
        if (array.length > 0) {
            itemsWithField++;
            array.forEach(val => {
                const value = getPatternValue(val);
                if (value) {
                    valueCounts[value] = (valueCounts[value] || 0) + 1;
                }
            });
...
export function extractServerDataPattern(items, serverField, defaultOperator, totalCount) {
    const valueCounts = {};
    let matchCount = 0;
...
export function extractYearPattern(items, totalCount) {
    const years = items
        .map(item => item.year)
...
export function calculateMatchPercentage(pattern, items) {
    let matches = 0;

    items.forEach(item => {
        if (itemMatchesPattern(item, pattern)) {
            matches++;
        }
    });
...
export function itemMatchesPattern(item, pattern) {
        ...
        case 'is_one_of':
            if (Array.isArray(itemValue)) {
                return itemValue.some(v =>
                    values.includes(getPatternValue(v))
                );
            }
            return values.includes(itemValue);
        case 'contains':
