...
const assessable = value => ['familiar', 'unusual'].includes(value?.status) &&
...
export function assessLeaderChallengeAcceptance(assessment, calibration) {
  ...
  if (!Array.isArray(pool) || pool.length < 2 || pool.length > 64 || new Set(pool).size !== pool.length ||
      ...
      match?.version !== LIBRARY_MATCH_BASELINE_VERSION || neighbor?.version !== NEIGHBOR_CROSS_FIT_VERSION ||
  ...
  if (!assessable(incumbent)) return result('incumbent_unassessable', incumbent, challenger);
  if (!assessable(challenger)) return result('challenger_unassessable', incumbent, challenger);
...
export function applyLeaderChallengeAcceptance(assessment, calibration) {
  const acceptance = assessLeaderChallengeAcceptance(assessment, calibration);
