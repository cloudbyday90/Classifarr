...
import { projectLearnedEvidenceGuardReasons } from './learnedEvidenceGuardReasons.mjs';
...
export function readLibraryEvaluationSummary(readStatus) {
  ...
  try {
    ...
    const guardReasons = projectLearnedEvidenceGuardReasons(source.guardReasons, counts.live_guard_blocked);
    return { ...unavailable, status: 'available', counts,
      ...(guardReasons ? { guardReasons } : {}), ...(representative ? { representative } : {}) };
  } catch {
    return unavailable;
  }
}
