  ...
  async collectBackupData(options = {}) {
    ...
    try {
      ...
      const backup = {
        ...
        data: {
          users: users.rows.map(u => ({ ...u, password_hash: '<excluded>' })), // Don't export password hashes
  ...
  async restoreBackup(filename, options = {}) {
    ...
    try {
      ...
      if (mode === 'replace') {
        ...
        await client.query('DELETE FROM libraries WHERE id > 0'); // Keep id sequence
      ...
      // Restore library policies with remapped IDs
      ...
      // Restore auto-learned preferences
      ...
      // Restore learning patterns if included
