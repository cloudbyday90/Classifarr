...
async function verifyPolicyRuntimeDestinationEvidenceFinalOutcome({
  client,
  command,
  executionState,
} = {}) {
  ...
  const currentState = asObject(state.currentState);
  const resolution = asObject(state.resolution);

  if (!client || typeof client.query !== 'function') {
    throw new TypeError('Runtime destination-evidence verification requires a transaction client.');
  }
  if (operation.operationId !==
      ...
      normalizeString(source.sourceEventId, 160) !==
        normalizeString(currentState.sourceEventId, 160)) {
    throw new TypeError('Runtime destination evidence requires a matching locked final outcome.');
  }
