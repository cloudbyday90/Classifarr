...
const POLICY_AUTHORING_PRESENTATION_TEST_INVENTORY_FILE_PATHS = deepFreeze([
  ...
  'client/src/__tests__/PolicyNativeIntentPurposeChangeSurface.test.js',
...
const POLICY_AUTHORING_PRESENTATION_TEST_EXCLUSION_RECORDS = deepFreeze([
  ...
  {
    filePath: 'client/src/__tests__/PolicyCandidateCorrectionAnalyticsStats.test.js',
    rationale: 'The aggregate correction analytics monitor is a read-only Statistics surface, not policy authoring or an adjacent authoring recovery flow.',
  },
]);
...
const POLICY_AUTHORING_PRESENTATION_TEST_RECORDS = deepFreeze([
  ...
  presentationTestRecord(
    ...
    [],
    'Native purpose maintenance reads only the current server-owned command and revision, treats coverage as advisory, and submits an explicit revision-bound update without reopening compatibility authoring or invoking AI, routing, or learning.'
  ),
  presentationTestRecord(
    'client/src/__tests__/PolicyHistoricRouteSafetyRefresh.test.js',
    POLICY_AUTHORING_PRESENTATION_TEST_CATEGORY_IDS.KEEP_WORKFLOW_REGRESSION,
  ...
  presentationTestRecord(
    ...
    POLICY_AUTHORING_PRESENTATION_TEST_OWNER_IDS.RUNTIME_VERIFIER,
    [],
    'The lifecycle provenance receipt renders only fixed aggregate counts and cannot expose a receipt identity, configuration, media item, semantic selection, or routing authority.'
  ),
  presentationTestRecord(
    'client/src/__tests__/PolicyScopedEvidenceDigest.test.js',
    POLICY_AUTHORING_PRESENTATION_TEST_CATEGORY_IDS.KEEP_WORKFLOW_REGRESSION,
