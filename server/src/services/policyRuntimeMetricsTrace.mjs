...
const POLICY_RUNTIME_METRIC_COMPONENT_IDS = Object.freeze({
...
const POLICY_RUNTIME_METRIC_REASON_IDS = Object.freeze({
...
const POLICY_RUNTIME_METRIC_AUDIT_RISK_IDS = Object.freeze({
...
const SOURCE_FINGERPRINT_TRACE_ATTRIBUTE = 'classifarr.policy.runtime_metrics_trace.source_fingerprint';
...
const COMPONENT_IDS = Object.freeze(Object.values(POLICY_RUNTIME_METRIC_COMPONENT_IDS));
...
function sensitiveReasonForEvent(event = {}) {
  ...
  if (hasOwnKeyDeep(event, ['embedding', 'embeddings', 'vector'])) {
    return POLICY_RUNTIME_METRIC_AUDIT_RISK_IDS.EMBEDDING_EXPOSED;
  }
  if (hasOwnKeyDeep(event, ['providerPayload'])) {
    return POLICY_RUNTIME_METRIC_AUDIT_RISK_IDS.PROVIDER_PAYLOAD_EXPOSED;
  }
  ...
  if (hasOwnKeyDeep(event, DIAGNOSTIC_KEYS)) {
    return POLICY_RUNTIME_METRIC_AUDIT_RISK_IDS.DIAGNOSTIC_INTERNAL_EXPOSED;
  }
  return null;
}
...
function buildTraceRecord({
  ...
  const attributes = {
    ...
    'classifarr.policy.runtime_metrics_trace.component': componentId,
    'classifarr.policy.runtime_metrics_trace.outcome': normalizeString(outcomeId),
    'classifarr.policy.runtime_metrics_trace.reason_count': reasonRecords.length,
    'classifarr.policy.runtime_metrics_trace.sensitive_suppressed': Boolean(sensitiveRiskId),
  };
  ...
  return {
    ...
    reasons: sensitiveRiskId
      ? [
        ...reasonRecords,
        {
          reasonId: POLICY_RUNTIME_METRIC_REASON_IDS.RAW_PAYLOAD_SUPPRESSED,
          severity: 'warning',
        },
...
function processAutomationDecision(counters, traces, decision = {}) {
  ...
  switch (stateId) {
    case POLICY_AUTOMATION_DECISION_STATE_IDS.AUTO_ROUTE_READY:
      counterIds.push(POLICY_RUNTIME_METRIC_COUNTER_IDS.AUTO_ROUTED);
      break;
    case POLICY_AUTOMATION_DECISION_STATE_IDS.CLASSIFIED_NOT_ROUTED:
      counterIds.push(POLICY_RUNTIME_METRIC_COUNTER_IDS.CLASSIFIED_NOT_ROUTED);
      break;
    case POLICY_AUTOMATION_DECISION_STATE_IDS.NEEDS_OPERATOR_REVIEW:
      counterIds.push(POLICY_RUNTIME_METRIC_COUNTER_IDS.ASKED_FOR_REVIEW);
      break;
    case POLICY_AUTOMATION_DECISION_STATE_IDS.BLOCKED_BY_HARD_LIMIT:
      counterIds.push(POLICY_RUNTIME_METRIC_COUNTER_IDS.BLOCKED_BY_HARD_LIMIT);
      break;
    case POLICY_AUTOMATION_DECISION_STATE_IDS.NEEDS_ROUTING_MAPPING:
      counterIds.push(POLICY_RUNTIME_METRIC_COUNTER_IDS.MISSING_ROUTING);
      break;
    case POLICY_AUTOMATION_DECISION_STATE_IDS.STALE_PROFILE_RETRY:
      counterIds.push(POLICY_RUNTIME_METRIC_COUNTER_IDS.STALE_PROFILE_RETRY);
      break;
    default:
      break;
  }
  ...
  traces.push(buildTraceRecord({
    ...
    outcomeId: stateId,
    counterIds,
    fallbackReasonId: POLICY_RUNTIME_METRIC_REASON_IDS.AUTOMATION_DECISION_COUNTED,
  }));
}
...
function processQuestionReduction(counters, traces, plan = {}) {
  ...
  counterIds.forEach(counterId => increment(counters, counterId));
  traces.push(buildTraceRecord({
    componentId: POLICY_RUNTIME_METRIC_COMPONENT_IDS.QUESTION_REDUCTION,
    source: plan,
    outcomeId: plan.dispositionId,
    counterIds,
    fallbackReasonId: POLICY_RUNTIME_METRIC_REASON_IDS.QUESTION_DISPOSITION_COUNTED,
  }));
}
...
function processRequestLearning(counters, traces, decision = {}) {
  ...
  switch (decision.dispositionId) {
    case POLICY_REQUEST_LEARNING_DISPOSITION_IDS.LEARNING_CANDIDATE:
      counterIds.push(POLICY_RUNTIME_METRIC_COUNTER_IDS.LEARNING_ALLOWED);
      break;
    case POLICY_REQUEST_LEARNING_DISPOSITION_IDS.BLOCKED:
    case POLICY_REQUEST_LEARNING_DISPOSITION_IDS.POLICY_EDIT_REQUIRED:
      counterIds.push(POLICY_RUNTIME_METRIC_COUNTER_IDS.LEARNING_BLOCKED);
      break;
    case POLICY_REQUEST_LEARNING_DISPOSITION_IDS.OUTCOME_ONLY:
    case POLICY_REQUEST_LEARNING_DISPOSITION_IDS.ROUTE_FAILURE_ONLY:
      counterIds.push(POLICY_RUNTIME_METRIC_COUNTER_IDS.LEARNING_DOWNGRADED);
      break;
    default:
      break;
  }
  ...
  counterIds.forEach(counterId => increment(counters, counterId));
  traces.push(buildTraceRecord({
    componentId: POLICY_RUNTIME_METRIC_COMPONENT_IDS.REQUEST_LEARNING,
    source: decision,
    outcomeId: decision.dispositionId,
    counterIds,
    fallbackReasonId: POLICY_RUNTIME_METRIC_REASON_IDS.REQUEST_LEARNING_COUNTED,
  }));
}
...
function processRebuildProposal(counters, traces, proposal = {}) {
  ...
  const counterIds = accepted
    ? [POLICY_RUNTIME_METRIC_COUNTER_IDS.REBUILD_ACCEPTED]
    : proposal.statusId === POLICY_REBUILD_PROPOSAL_STATUS_IDS.BLOCKED
      ? [POLICY_RUNTIME_METRIC_COUNTER_IDS.REBUILD_REJECTED]
      : [];

  counterIds.forEach(counterId => increment(counters, counterId));
  traces.push(buildTraceRecord({
    componentId: POLICY_RUNTIME_METRIC_COMPONENT_IDS.REBUILD_PROPOSAL,
    source: proposal,
    outcomeId: accepted ? POLICY_REBUILD_EVENT_STATUS_IDS.ACCEPTED : proposal.statusId,
    counterIds,
    fallbackReasonId: POLICY_RUNTIME_METRIC_REASON_IDS.REBUILD_PROPOSAL_COUNTED,
  }));
}
...
function processMigrationVerifier(counters, traces, report = {}) {
  ...
  if (report.applicationGate?.operatorAccepted === true) {
    counterIds.push(POLICY_RUNTIME_METRIC_COUNTER_IDS.REBUILD_ACCEPTED);
  }
  if (report.statusId === POLICY_MIGRATION_VERIFIER_STATUS_IDS.BLOCKED_BY_MIGRATION_RISK) {
    counterIds.push(POLICY_RUNTIME_METRIC_COUNTER_IDS.REBUILD_REJECTED);
  }

  counterIds.forEach(counterId => increment(counters, counterId));
  traces.push(buildTraceRecord({
    componentId: POLICY_RUNTIME_METRIC_COMPONENT_IDS.MIGRATION_VERIFIER,
    source: report,
    outcomeId: report.statusId,
    counterIds,
    fallbackReasonId: POLICY_RUNTIME_METRIC_REASON_IDS.MIGRATION_VERIFIER_COUNTED,
  }));
}
...
function processRebuildEvent(counters, traces, event = {}) {
  ...
  if (statusId === POLICY_REBUILD_EVENT_STATUS_IDS.ROLLED_BACK) {
    counterIds.push(POLICY_RUNTIME_METRIC_COUNTER_IDS.REBUILD_ROLLED_BACK);
  }

  counterIds.forEach(counterId => increment(counters, counterId));
  traces.push(buildTraceRecord({
    componentId: POLICY_RUNTIME_METRIC_COMPONENT_IDS.REBUILD_EVENT,
    source: event,
    outcomeId: statusId,
    counterIds,
    fallbackReasonId: POLICY_RUNTIME_METRIC_REASON_IDS.REBUILD_EVENT_COUNTED,
  }));
}
...
function buildOperatorSummary(counters) {
  ...
  if (counters[POLICY_RUNTIME_METRIC_COUNTER_IDS.MISSING_ROUTING] > 0) {
    summaries.push({
      actionId: 'configure_routing',
      label: 'Configure routing for destinations that cannot route yet.',
      counterId: POLICY_RUNTIME_METRIC_COUNTER_IDS.MISSING_ROUTING,
    });
  }
  if (counters[POLICY_RUNTIME_METRIC_COUNTER_IDS.ASKED_FOR_REVIEW] > 0) {
    summaries.push({
      actionId: 'review_pending_items',
      label: 'Review pending items that need operator confirmation.',
      counterId: POLICY_RUNTIME_METRIC_COUNTER_IDS.ASKED_FOR_REVIEW,
    });
  }
  if (counters[POLICY_RUNTIME_METRIC_COUNTER_IDS.STALE_PROFILE_RETRY] > 0) {
    summaries.push({
      actionId: 'refresh_profile',
      label: 'Refresh stale library profiles before trusting automation.',
      counterId: POLICY_RUNTIME_METRIC_COUNTER_IDS.STALE_PROFILE_RETRY,
    });
  }
  if (counters[POLICY_RUNTIME_METRIC_COUNTER_IDS.REBUILD_REJECTED] > 0) {
    summaries.push({
      actionId: 'review_rebuild_verifier',
      label: 'Review rebuild verifier blockers before replacement.',
      counterId: POLICY_RUNTIME_METRIC_COUNTER_IDS.REBUILD_REJECTED,
    });
  }
...
function validatePolicyRuntimeMetricsTrace(metrics = {}) {
  ...
  [
    ['exposesRawPayload', POLICY_RUNTIME_METRIC_AUDIT_RISK_IDS.RAW_PAYLOAD_EXPOSED],
    ['exposesPrompt', POLICY_RUNTIME_METRIC_AUDIT_RISK_IDS.PROMPT_EXPOSED],
    ['exposesEmbedding', POLICY_RUNTIME_METRIC_AUDIT_RISK_IDS.EMBEDDING_EXPOSED],
    ['exposesProviderPayload', POLICY_RUNTIME_METRIC_AUDIT_RISK_IDS.PROVIDER_PAYLOAD_EXPOSED],
    ['exposesDiagnosticInternal', POLICY_RUNTIME_METRIC_AUDIT_RISK_IDS.DIAGNOSTIC_INTERNAL_EXPOSED],
  ].forEach(([key, riskId]) => {
