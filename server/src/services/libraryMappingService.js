...
class LibraryMappingService {
    ...
    async getArrRootFolders(arrType, arrConfigId) {
        ...
        const configResult = await db.query(`SELECT * FROM ${table} WHERE id = $1`, [arrConfigId]); // sql-interpolation: table is a ternary constant ('radarr_config'|'sonarr_config'), not user-controlled
    ...
    async linkArrToMediaServer(arrType, arrConfigId, mediaServerId) {
        ...
        await db.query(`UPDATE ${table} SET media_server_id = $1 WHERE id = $2`, [mediaServerId, arrConfigId]); // sql-interpolation: table is a ternary constant ('radarr_config'|'sonarr_config'), not user-controlled
