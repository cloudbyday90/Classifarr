...
class PolicyQuestionBuilder {
  ...
  get mergePresetSignals() {
    if (!this._mergePresetSignals) {
      ({ mergePresetSignals: this._mergePresetSignals } = require('../utils/policySignals'));
    }
  ...
  get normalizeSignalConfig() {
    if (!this._normalizeSignalConfig) {
      ({ normalizeSignalConfig: this._normalizeSignalConfig } = require('../utils/policySignals'));
    }
  ...
  get normalizeMetadataList() {
    if (!this._normalizeMetadataList) {
      ({ normalizeMetadataList: this._normalizeMetadataList } = require('../utils/metadataNormalization'));
    }
  ...
  buildQuestionPayload(metadata, { problem_summary, why_uncertain, question, options, candidates, extras = {} }) {
    ...
    const tags = {
      genres: this.normalizeMetadataList(metadata.genres),
      keywords: this.normalizeMetadataList(metadata.keywords).slice(0, 10)
    };
    ...
    if (
      ...
      this.logger.warn('Policy question option order diverges from primary candidate', {
...
module.exports.PolicyQuestionBuilder = PolicyQuestionBuilder;
