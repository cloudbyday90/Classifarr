...
class JellyfinService {
  ...
  async getLibraryItems(url, apiKey, libraryKey, options = {}) {
    ...
    try {
      ...
      return items.map(item => this.mapJellyfinItem(item));
    } catch (error) {
  ...
  async searchByProviderIds(url, apiKey, tmdbId) {
    try {
      ...
      const items = response.data.Items || [];
      if (items.length > 0) {
        return this.mapJellyfinItem(items[0]);
      }
      return null;
    } catch (error) {
      return null;
  ...
  /**
   * Map Jellyfin item to standard format
   */
  mapJellyfinItem(item) {
    ...
    return {
      ...
      genres: item.Genres || [],
      tags: item.Tags || [],
      collections: [], // Jellyfin doesn't provide this in item data
