...
class JellyfinService {
  ...
  async getCollections(url, apiKey, parentId) {
    try {
      ...
      throw new Error(`Failed to fetch Jellyfin collections: ${error.message}`);
    }
  }
  ...
  async getCollectionItems(url, apiKey, collectionId) {
    try {
      ...
      throw new Error(`Failed to fetch Jellyfin collection items: ${error.message}`);
    }
  }
  ...
  async searchByProviderIds(url, apiKey, tmdbId) {
    try {
      ...
      throw new Error(`Failed to search Jellyfin by provider IDs: ${error.message}`);
    }
  }
  ...
  /**
   * Helper: Normalize Jellyfin item to standard format
   */
  normalizeItem(item, includeFullMetadata = false) {
    ...
    const normalized = {
      ...
      genres: (item.Genres || []),
      tags: (item.Tags || []),
      collections: [], // Jellyfin doesn't expose collections in item metadata directly
