 ...
 *
 * Owns the RAG-loop second-pass orchestration and related helpers:
 *   - getRagLoopConfig / getCurrentAppVersion / getCurrentImageTag
 *   - getRecentFallbackDiagnostics / buildAutoFallbackIncidentPayload
 *   - persistAutoFallbackBreachCount / maybeApplyRolloutAutomation
 *   - buildFreshSecondPassBaseResult / buildPolicyRecheckCandidate / buildAiRerunCandidate
...
class ClassificationRagLoopService {
  ...
  // ---------------------------------------------------------------------------
  // Second-pass candidate builders
  // ---------------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------------
  // Main RAG loop second-pass orchestration
  // ---------------------------------------------------------------------------
  ...
  async evaluateRagLoopSecondPass({
    ...
    for (let attempt = 1; attempt <= pass1MaxAttempts; attempt += 1) {
      try {
        pass1Candidates = await classificationUtilsService.withTimeout(
      ...
      if (!resilienceGate.allowed) {
        ...
        for (let attempt = 1; attempt <= enrichmentMaxAttempts; attempt += 1) {
          try {
            ...
            const enrichedMetadata = await classificationUtilsService.withTimeout(
              classificationMetadataService.enrichWithTMDB(workingMetadata.tmdb_id, workingMetadata.media_type),
              timeoutMs,
        ...
        for (let attempt = 1; attempt <= pass2MaxAttempts; attempt += 1) {
          try {
            if (strategySelection.strategy === 'semantic') {
              pass2Matches = await classificationUtilsService.withTimeout(
    ...
    if (pass2Enabled && remainingBudget() > 0) {
      ...
      for (let attempt = 1; attempt <= pass2CandidateMaxAttempts; attempt += 1) {
        try {
          pass2Candidates = await classificationUtilsService.withTimeout(
    ...
    if ((trigger.trigger === 'policy_prompt_select' || trigger.trigger === 'policy_prompt_confirm') && remainingBudget() > 0) {
      ...
      if (policyRecheckMaxAttempts <= 0) {
        ...
        for (let attempt = 1; attempt <= policyRecheckMaxAttempts; attempt += 1) {
          try {
            policyAfter = await classificationUtilsService.withTimeout(
    ...
    if (!pass2Candidate) {
      ...
      if (!resilienceGate.allowed) {
        ...
        if (aiRerunGate.eligible) {
          try {
            aiCallsUsed += 1;
            const aiRerunMatch = await classificationAiService.aiClassify(expandedMetadata, libraries, signalContext, {
              mode: 'verify',
              ragContext: pass2RagContext
            });
...
module.exports = new ClassificationRagLoopService();
