...
// Mock ragLoopHelpers exports used by withRetryableDbConflict.
...
describe('resolveRagLoopTimeout', () => {
  ...
  test('clamps result to RAG_LOOP_MIN_TIMEOUT_MS (1000) when very small', () => {
    // -10000 + 8000 = -2000 → clamped to 1000
...
describe('withTimeout', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    // Default: controller.run just calls the fn
  ...
  test('runs promise directly when operationOrPromise is already a Promise', async () => {
    ...
    // OperationController should not be used for plain promises
...
describe('withRetryableDbConflict', () => {
  beforeEach(() => {
    // Use mockReset (not just clearAllMocks) so that mockReturnValueOnce queues
    // from previous tests don't leak into this test's service calls.
  ...
  test('retries on retryable conflict error and succeeds on second attempt', async () => {
    ...
    isRetryableDbConflictError.mockReturnValueOnce(true); // only set the one item we need; default (false) covers the rest
...
describe('buildPendingRetryResult', () => {
  ...
  test('returns correct base shape with defaults', () => {
    ...
    expect(result).toMatchObject({
      ...
      libraries: [],
      signalContext: null,
      // null previousRetryCount → Number(null)===0 → normalised to 0 → retry_count = 0+1 = 1
      retry_count: 1,
      max_retries: 3,
    });
  });
  ...
  // Number(null) === 0, which is a valid integer >= 0, so it normalises to 0
  // and retry_count becomes 0 + 1 = 1. Pass `undefined` if you truly want 0.
