...
} from '../../services/policyBuilderPhase8ExecutionPlanArtifact.mjs';
...
describe('policyBuilderPhase8ExecutionPlanArtifact', () => {
  test('builds a ready execution-plan artifact from explicit input evidence', () => {
    ...
    expect(artifact.statusId).toBe(PHASE8R_EXECUTION_PLAN_ARTIFACT_STATUS_IDS.READY);
  ...
  test('blocks artifact readiness when approval evidence is missing', () => {
    ...
    expect(artifact.statusId).toBe(PHASE8R_EXECUTION_PLAN_ARTIFACT_STATUS_IDS.BLOCKED);
  ...
  test('validates artifact invariants', () => {
    ...
    const validation = validatePolicyBuilderPhase8ExecutionPlanArtifact({
      ...artifact,
      statusId: 'unknown',
      riskCount: 999,
    });
