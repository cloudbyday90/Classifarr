...
describe('policyPurposeEvidenceInventory', () => {
  test('fails closed when no active authoritative native policy exists', () => {
    expect(buildPolicyPurposeEvidenceInventory()).toEqual({
      version: 'policy_purpose_evidence_inventory.v3',
  ...
  test('accepts a declared current intent with a matching receipt even when historic receipts differ', () => {
    expect(buildPolicyPurposeEvidenceInventory({
      ...
      normal_lifecycle_receipt_policy_count: 4,
      verifiable_lifecycle_receipt_policy_count: 4,
  ...
  test('bounds malformed and contradictory aggregate counts', () => {
    expect(buildPolicyPurposeEvidenceInventory({
      ...
      current_intent_retained_purpose_lifecycle_receipt_policy_count: 99,
