 ...
 *
 * Tests for ClassificationEvidenceComparisonService.
 */

const {
  ClassificationEvidenceComparisonService,
  MISMATCH_REASON
} = require('../../services/classificationEvidenceComparisonService');
  ...
  describe('buildComparisonRecord', () => {
    ...
    test('reports not consistent when either sub-comparison fails', async () => {
      const svc = makeSvc(
        { findExactMatch: jest.fn().mockResolvedValue({ library_id: 7 }) },
        {
          findExactMatch: jest.fn().mockResolvedValue({ library_id: 9 }),  // mismatch
          findRelatedEvidence: jest.fn().mockResolvedValue([])
        }
      );
