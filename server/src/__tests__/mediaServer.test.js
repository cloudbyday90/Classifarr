...
describe('Media Server API', () => {
    let app;
    let mockClient;
    let createMediaServerRouter;
    let plexService;
    let embyService;
    let jellyfinService;

    beforeAll(async () => {
        ({ createMediaServerRouter } = await import('../routes/mediaServer.mjs'));
        ({ plexService, embyService, jellyfinService } = await import('../services/mediaServers/index.mjs'));
    });
