...
const { QueueTavilyEnrichmentService } = require('../services/queueTavilyEnrichmentService');
...
describe('enrich — no config', () => {
  test('returns enrichmentData unchanged when no active tavily config', async () => {
    const db = makeDb();
    db.query.mockResolvedValueOnce({ rows: [] }); // no active config
    ...
    expect(tavilyService.getContentAdvisory).not.toHaveBeenCalled();
  });
...
describe('enrich — advisory enrichment', () => {
  test('adds tavily_advisory when content advisory found', async () => {
    ...
    tavilyService.getContentAdvisory.mockResolvedValueOnce({
      results: [{ content: 'Some advisory content' }],
      answer: 'PG-13 for action'
    });
    tavilyService.search.mockResolvedValueOnce({ answer: null }); // holiday
    normalizeMetadataListLower.mockReturnValueOnce([]); // not anime
  ...
  test('skips advisory when no results', async () => {
    ...
    tavilyService.getContentAdvisory.mockResolvedValueOnce({ results: [], answer: null });
    tavilyService.search.mockResolvedValueOnce({ answer: null });
  ...
  test('swallows advisory error and continues', async () => {
    const db = makeDb();
    db.query.mockResolvedValueOnce({ rows: [{ api_key: 'key123' }] });
    tavilyService.getContentAdvisory.mockRejectedValueOnce(new Error('API down'));
    tavilyService.search.mockResolvedValueOnce({ answer: null });
    normalizeMetadataListLower.mockReturnValueOnce([]);
...
describe('enrich — holiday enrichment', () => {
  test('adds tavily_holiday when answer found', async () => {
    ...
    db.query.mockResolvedValueOnce({ rows: [{ api_key: 'key123' }] });
    tavilyService.getContentAdvisory.mockResolvedValueOnce({ results: [], answer: null });
    tavilyService.search.mockResolvedValueOnce({ answer: 'Yes it is a holiday film' });
    normalizeMetadataListLower.mockReturnValueOnce([]);
...
describe('enrich — anime enrichment', () => {
  test('adds tavily_anime for Japanese language content', async () => {
    ...
    tavilyService.getContentAdvisory.mockResolvedValueOnce({ results: [], answer: null });
    tavilyService.search.mockResolvedValueOnce({ answer: null }); // holiday
    normalizeMetadataListLower.mockReturnValueOnce(['action']); // no 'anime' in genres
    tavilyService.searchAnimeInfo.mockResolvedValueOnce({
      results: [
    ...
    expect(result.tavily_anime.results[0].snippet).toHaveLength(500); // truncated to 500
  });
  ...
  test('adds tavily_anime when genres contain anime', async () => {
    ...
    tavilyService.getContentAdvisory.mockResolvedValueOnce({ results: [], answer: null });
    tavilyService.search.mockResolvedValueOnce({ answer: null });
    normalizeMetadataListLower.mockReturnValueOnce(['anime', 'action']); // genres contain 'anime'
    tavilyService.searchAnimeInfo.mockResolvedValueOnce({
    ...
    expect(tavilyService.searchAnimeInfo).toHaveBeenCalled();
  });
  ...
  test('swallows anime search error', async () => {
    ...
    tavilyService.search.mockResolvedValueOnce({ answer: null });
    normalizeMetadataListLower.mockReturnValueOnce([]);
    tavilyService.searchAnimeInfo.mockRejectedValueOnce(new Error('Anime API failed'));
