...
import {
  runHeldOutSemanticStudyEligibilityAudit,
} from '../../scripts/runHeldOutSemanticStudyEligibilityAudit.mjs';

test('runs the private eligibility audit without file arguments and always closes its runtime', async () => {
  const close = jest.fn();
  const audit = { audit: jest.fn(async () => ({ version: 'test.v1', status: { id: 'complete' } })) };
