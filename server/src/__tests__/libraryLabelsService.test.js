...
const { matchRules, metadataMatchesLabel, evaluateCustomRule, evaluateSingleCondition } = require('../services/libraryLabelsService');
...
function makeDb(labelRows = [], ruleRows = []) {
    return {
        query: jest.fn()
            .mockResolvedValueOnce({ rows: labelRows })  // labels query
            .mockResolvedValueOnce({ rows: ruleRows }),  // custom rules query
    };
}
...
describe('matchRules', () => {
    ...
    it('selects the highest-scoring library across multiple libraries', async () => {
        ...
        expect(result.confidence).toBe(55); // 25 label + 30 rule
    });
...
describe('evaluateCustomRule', () => {
    ...
    it('returns false on thrown error', () => {
        // Pass null condition to trigger an error path
