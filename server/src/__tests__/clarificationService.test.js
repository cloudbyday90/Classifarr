...
jest.mock('../services/classificationOutcomeService', () => ({ recordOutcome: jest.fn() }));
  ...
  test('extracts valid integer library_ids', () => {
    const question = {
      options: [
        { library_id: '5' },
        { library_id: '10' },
        { library_id: '0' },   // filtered out (not > 0)
        { library_id: 'abc' }, // filtered out (NaN)
        { library_id: null }   // filtered out
      ]
    };
...
// ---------------------------------------------------------------------------
// getTierFromPolicyThresholds (pure)
// ---------------------------------------------------------------------------
...
describe('getTierFromPolicyThresholds', () => {
  ...
  test('requireAllConfirmations=true forces verify even at auto-level confidence', () => {
    ...
    // auto path is skipped, so verify or null depending on prompt threshold
...
describe('getTierForConfidence', () => {
  test('returns matching tier from DB', async () => {
    ...
    expect(db.query).toHaveBeenCalledWith(expect.any(String), [90]); // rounds 90 to 90
  });
...
// ---------------------------------------------------------------------------
// isRequireAllConfirmationsEnabled
// ---------------------------------------------------------------------------
...
describe('matchQuestions', () => {
  ...
  test('returns [] on DB error', async () => {
    db.query
      .mockResolvedValueOnce({ rows: [] })  // hasLanguagePresets
  ...
  test('returns [] when no questions score > 0', async () => {
    db.query
      .mockResolvedValueOnce({ rows: [] })  // hasLanguagePresets → false
  ...
  test('language question gets 40-point boost when language is allowed', async () => {
    normalizeMetadataListLower.mockReturnValue([]);
    db.query
      .mockResolvedValueOnce({ rows: [{ 1: 1 }] }) // hasLanguagePresets → true
      .mockResolvedValueOnce({
...
describe('createQuestion', () => {
  ...
  test('defaults trigger_keywords to [], priority to 0, enabled to true', async () => {
    ...
    expect(params[2]).toEqual([]); // trigger_keywords
    expect(params[3]).toEqual([]); // trigger_genres
    expect(params[5]).toBe(0);     // priority
...
describe('recordResponse', () => {
  test('records response, updates classification, returns result', async () => {
    ...
    db.query
      .mockResolvedValueOnce(question)               // question lookup
      .mockResolvedValueOnce({ rows: [insertedRow] }) // INSERT response
      .mockResolvedValueOnce({ rows: [] });            // UPDATE classification

    const result = await svc.recordResponse(5, 1, 'yes', 'discordUser1', 60);
    expect(result.success).toBe(true);
    expect(result.confidenceAfter).toBe(80); // 60 + 20
    expect(result.response).toEqual(insertedRow);
    expect(result.shouldReclassify).toBe(true); // 80 >= 70
  });
...
describe('resolvePolicyQuestion', () => {
  test('resolves successfully when classification is awaiting_decision', async () => {
    ...
    client.query
      ...
      .mockResolvedValueOnce({ rows: [classification] })                      // FOR UPDATE
      .mockResolvedValueOnce({ rows: [{ id: 5, name: 'Movies', arr_type: 'radarr', media_type: 'movie', is_active: true }] }) // library
  ...
  test('throws 404 when classification does not exist', async () => {
    ...
    client.query
      .mockResolvedValueOnce({})              // BEGIN
      .mockResolvedValueOnce({ rows: [] })    // FOR UPDATE → not found
  ...
  test('returns alreadyResolved=true when status is completed with same library', async () => {
    ...
    client.query
      .mockResolvedValueOnce({})              // BEGIN
      .mockResolvedValueOnce({ rows: [] })    // FOR UPDATE → not awaiting
      .mockResolvedValueOnce({ rows: [{ id: 5 }] }) // library check
      .mockResolvedValueOnce({ rows: [{ status: 'completed', library_id: '5', library_name: 'Movies' }] }) // existence check
      .mockResolvedValueOnce({});             // COMMIT
  ...
  test('throws 409 when classification is no longer awaiting_decision (different state)', async () => {
    ...
    client.query
      ...
      .mockResolvedValueOnce({ rows: [] })    // FOR UPDATE → not awaiting
      .mockResolvedValueOnce({ rows: [{ id: 5 }] }) // library check
      .mockResolvedValueOnce({ rows: [{ status: 'pending_retry', library_id: '99', library_name: 'Other' }] }); // existence
  ...
  test('throws 400 when library is inactive', async () => {
    ...
    client.query
      ...
      .mockResolvedValueOnce({ rows: [{ id: 5, name: 'Movies', arr_type: 'radarr', media_type: 'movie', is_active: false }] }); // inactive
