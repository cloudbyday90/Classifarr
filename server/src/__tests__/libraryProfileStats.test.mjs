...
describe('LibraryProfileService - Profile Statistics', () => {
    ...
    it('projects every live distribution from one observation snapshot', async () => {
        db.query.mockResolvedValueOnce({ rows: [
            { observed_item_id: 1, tmdb_id: 7, media_type: 'movie', genres: ['Action', 'Drama', 'Action'], content_rating: 'PG', studio: 'A', metadata: { inventory_tmdb: { version: 1, tmdb_id: 7, media_type: 'movie', keywords: [], original_language: 'en' } } },
            { observed_item_id: 2, genres: ['Action'], content_rating: 'G', metadata: {} },
        ] });
    ...
    it.each(['getCertificationDistribution', 'getGenreDistribution', 'getStudioDistribution', 'getLanguageDistribution'])('uses the shared population for %s', async method => {
        db.query.mockResolvedValueOnce({ rows: [{ observed_item_id: 1, tmdb_id: 7, media_type: 'movie', genres: ['Action'], content_rating: 'PG', studio: 'A', metadata: { inventory_tmdb: { version: 1, tmdb_id: 7, media_type: 'movie', keywords: [], original_language: 'en' } } }, { observed_item_id: 2 }] });
