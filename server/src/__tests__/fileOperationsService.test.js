...
const svc = require('../services/fileOperationsService');
...
describe('copyFileWithPermissions', () => {
  test('copies file and returns success=true', async () => {
    jest.spyOn(svc, 'getStats')
      .mockResolvedValueOnce({ exists: true, isFile: true, size: 500, mode: 0o644, uid: 1000, gid: 1000, atime: new Date(), mtime: new Date() }) // src
      .mockResolvedValueOnce({ exists: true, isFile: true, size: 500, mode: 0o644, uid: 1000, gid: 1000 }); // dest
