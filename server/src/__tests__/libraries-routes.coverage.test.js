...
describe('Libraries routes coverage', () => {
  ...
  test('POST /api/libraries/sync-arr-profiles syncs radarr and sonarr profile cache', async () => {
    db.query
      ...
      .mockResolvedValue({ rows: [{ id: 2, url: 'http://sonarr', api_key: 'sk', is_active: true }] }); // sonarr configs + inserts
  ...
  test('GET /api/libraries/:id/rules/suggest builds heuristic suggestions', async () => {
    db.query
      ...
      .mockResolvedValueOnce({ rows: [] }) // existing rules
      .mockResolvedValueOnce({
  ...
  describe('GET /api/libraries/:id/rules/smart-suggest', () => {
    ...
    test('returns llm suggestions and filters content_type condition', async () => {
      db.query
        .mockResolvedValueOnce({ rows: [{ id: 4, name: 'Anime Family', media_type: 'movie' }] }) // library
    ...
    test('falls back to data-analysis when no active ollama config', async () => {
      db.query
        .mockResolvedValueOnce({ rows: [{ id: 4, name: 'Kids Anime', media_type: 'movie' }] }) // library
        ...
        .mockResolvedValueOnce({ rows: [] })
        .mockResolvedValueOnce({ rows: [] })
        .mockResolvedValueOnce({ rows: [] }); // no ollama config
  ...
  describe('auto-generate rule endpoints', () => {
    ...
    test('POST /:id/rules/auto-generate inserts inferred rules', async () => {
      db.query
        .mockResolvedValueOnce({ rows: [{ id: 9, name: 'Kids Anime Christmas' }] }) // library
        .mockResolvedValueOnce({ rows: [] }) // existing rules
        .mockResolvedValue({ rows: [] }); // insert rules
    ...
    test('POST /auto-generate-all processes active libraries', async () => {
      db.query
        .mockResolvedValueOnce({ rows: [{ id: 1 }, { id: 2 }] }) // active libraries
        .mockResolvedValueOnce({ rows: [{ count: '0' }] }) // library 1 has no rules
        .mockResolvedValueOnce({ rows: [{ id: 1, name: 'Family Movies' }] }) // library 1 record
        .mockResolvedValue({ rows: [{ count: '1' }] }); // subsequent checks/inserts
