...
describe('Policies routes coverage', () => {
  ...
  describe('POST /api/policies', () => {
    ...
    test('returns representative replay samples without provider reads or policy storage mutation', async () => {
      db.query
        .mockResolvedValueOnce({
          rows: [{
            id: 5,
      ...
      expect(res.body.sample.items[0]).not.toHaveProperty('id');
        ...
        expect(res.body.sample).not.toHaveProperty('enrichment_eligibility');
        expect(res.body.sample).not.toHaveProperty('provider_readiness');
        expect(res.body.sample).not.toHaveProperty('enrichment_adapter_contract');
        expect(res.body.sample).not.toHaveProperty('tmdb_metadata_adapter_preview');
        expect(res.body.sample).not.toHaveProperty('tmdb_metadata_coverage_comparison');
      });
