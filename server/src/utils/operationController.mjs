...
class OperationController {
    ...
    get _logger() {
        if (!this.logger) {
            this.logger = createLogger('OperationController');
        }
        return this.logger;
    }
    ...
    abort(reason = 'Operation aborted') {
        ...
        this._logger.debug('Operation aborted', {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                if (this.status === OperationController.Status.RUNNING) {
                    ...
                    this._logger.debug('Operation completed', {
                        operation: this._operationName,
                        elapsedMs: this.elapsedMs
                    });
                ...
                if (this.status === OperationController.Status.RUNNING) {
                    ...
                    if (error.name === 'AbortError' || this.isAborted) {
                        this.status = OperationController.Status.ABORTED;
                        this._logger.debug('Operation aborted via signal', {
                            operation: this._operationName,
                            elapsedMs: this.elapsedMs
                        });
                    } else {
                        this.status = OperationController.Status.COMPLETED;
                        this._logger.debug('Operation failed with error', {
                            operation: this._operationName,
                            elapsedMs: this.elapsedMs,
                            error: error.message
                        });
    ...
    _setupSimpleTimeout() {
        this._timeoutHandle = setTimeout(() => {
            ...
            this._logger.warn('Operation timed out', {
                operation: this._operationName,
    ...
    _setupStreamingTimeouts() {
        this._hardTimeoutHandle = setTimeout(() => {
            ...
            this._logger.warn('Operation hard timeout', {
                operation: this._operationName,
        ...
        this._heartbeatInterval = setInterval(() => {
            ...
            if (timeSinceLastActivity > currentTimeout) {
                ...
                if (this.partialResult !== null && this.allowPartialOnStall) {
                    ...
                    this._logger.info('Operation completed with partial result after stall', {
                        operation: this._operationName,
                    ...
                    this._logger.warn('Operation stalled', {
                        operation: this._operationName,
                        stalledForMs: timeSinceLastActivity,
