 ...
 *
 * USAGE
 *   node server/src/scripts/verify_classification_evidence_backfill.js
 *
 * EXIT CODES
 *   0 — all checks passed
...
async function countBySource(client) {
  ...
  return {
    ...
    total: result.rows.reduce((sum, r) => sum + r.cnt, 0)
  };
}
...
async function verify({ database = null } = {}) {
  ...
  try {
    ...
    // Check 2: row counts match source
    ...
    addCheck(
      ...
      malformed.length === 0
        ? 'OK'
        : `${malformed.length} malformed key(s): ${JSON.stringify(malformed.map((r) => r.evidence_key))}`
    );

    // Check 4: item_exact rows all have tmdb_id
    ...
    addCheck(
      ...
      missingTmdb.length === 0
        ? 'OK'
        : `${missingTmdb.length} item_exact row(s) have null tmdb_id (ids: ${missingTmdb.map((r) => r.id).join(', ')})`
    );

    // Check 5: no rows with unknown scope
    ...
    addCheck(
      ...
      badScope.rows.length === 0
        ? 'OK'
        : `Unknown scope(s): ${badScope.rows.map((r) => `${r.scope}(${r.cnt})`).join(', ')}`
    );
  ...
  verify()
    .then((report) => {
      ...
      if (report.passed) {
        ...
        const failures = report.checks.filter((c) => !c.passed).length;
