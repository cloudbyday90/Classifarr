...
function normalizeImageProviderMode(mode) {
    if (mode === 'local') {
...
function resolveImageStatus({ enabled, mode, providerConfigured, stats, config }) {
...
function resolveProviderOnline({ providerConfigured, circuitStatus, embeddingAvailability }) {
    return Boolean(providerConfigured)
        && circuitStatus?.state !== 'OPEN'
        && embeddingAvailability?.status === 'available';
}

function resolveImageProviderOnline({ mode, enabled, providerConfigured }) {
    return mode !== 'disabled' && Boolean(enabled) && Boolean(providerConfigured);
}

function resolveImageProvider({ mode, config }) {
    if (mode === 'disabled') {
        return 'disabled';
    }
