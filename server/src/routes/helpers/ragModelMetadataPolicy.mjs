...
export function normalizeTextModelMode(mode) {
...
export function resolveSelectedTextModelProvider({ mode, provider, config, embeddingProvider }) {
...
export function resolveTextModelApiKey({ mode, provider, apiKey, config, isMaskedToken }) {
...
export function resolveTextModelApiEndpoint({ mode, apiEndpoint, config }) {
    if (apiEndpoint) {
        return apiEndpoint;
    }
...
export function resolveImageModelsCacheForLookup({ config, mode, localHost, localPort, cloudProvider, cloudApiEndpoint }) {
...
export function resolveImageCloudApiKey({ apiKey, config, isMaskedToken }) {
    if (apiKey && !isMaskedToken(apiKey)) {
        return apiKey;
    }
