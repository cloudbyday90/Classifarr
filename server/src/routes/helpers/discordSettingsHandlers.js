...
function maskDiscordConfig(config) {
  if (!config) {
    return null;
  }
...
async function fetchDiscordConfig(dbOrClient) {
...
async function resolveDiscordBotToken(dbOrClient, submittedToken, { allowMissingFallback = false } = {}) {
...
function createDiscordSettingsHandlers({ db, service, logger }) {
  return {
