...
const { isMaskedToken, maskToken } = require('../../utils/tokenMasking');

async function fetchSingleProviderConfig(dbOrClient, table, { activeOnly = false } = {}) {
    const whereClause = activeOnly ? ' WHERE is_active = true' : '';
    const result = await dbOrClient.query(`SELECT * FROM ${table}${whereClause} LIMIT 1`);
    return result.rows[0] || null;
}

function maskProviderApiKey(config) {
    if (!config) {
        return null;
    }
...
async function resolveRequestApiKey({ dbOrClient, table, submittedApiKey, activeOnly = false, allowStoredFallback = false }) {
...
function buildUnavailableHealthResponse(message, extra = {}) {
    return {
        status: 'unavailable',
        configured: false,
        ssl_valid: null,
...
function buildErrorHealthResponse(error, extra = {}) {
    return {
        status: 'unavailable',
        configured: null,
        ssl_valid: null,
...
function buildHealthyProviderResponse(healthResult, extra = {}) {
    return {
