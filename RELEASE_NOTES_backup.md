# Classifarr Release Notes Archive

(Older releases moved from RELEASE_NOTES.md)
...
- **Auto-Healing Database**: Automatically detects and fixes vector dimension mismatches. Switching embedding models (e.g., 768d â†’ 1024d) now automatically resizes the database column without manual SQL.
...
#### Settings No Longer Reset When Saving RAG Config ðŸ”§
...
#### Accurate Provider Status in RAG Dashboard ðŸ“Š
...
- **Fix**: Status now correctly checks if provider is actually configured based on mode:
  - "Same as Classification" â†’ checks if `primary_provider` is configured
  - "Separate Ollama" â†’ checks if host is configured
  - "Cloud" â†’ checks if API key is configured

#### Backfill Services Check RAG Enabled âš™ï¸
...
#### RAG Embedding Connection Fixes ðŸ”Œ
...
#### RAG & Embeddings Settings Consolidation ðŸ§¹
...
- Heartbeat settings integrated into RAG â†’ Backfill tab
...
#### Robust Error Handling with Adaptive Timeouts & Retry Logic ðŸ›¡ï¸

Embedding API calls now include enterprise-grade error handling to improve system stability and resilience (#153).

**Intelligent Retry Logic:**

- **Exponential Backoff**: Automatic retry with increasing delays (e.g., 1s â†’ 2s â†’ 4s)
- **Jitter**: Randomization prevents thundering herd (configurable Â±30%)
...
- **States**: CLOSED (normal) â†’ OPEN (blocking) â†’ HALF_OPEN (testing recovery)
...
- **Model Status**: Visual indicator (ðŸ§Š cold / ðŸ”¥ warm) with warmup button
...
#### RAG Settings Dashboard ðŸŽ›ï¸

All RAG configuration is now consolidated in one comprehensive page: **Settings â†’ RAG Settings**
...
- Toggle on/off in Settings â†’ AI â†’ Backfill

**2. Idle Backfill Mode ðŸŒ™**
...
**Configuration:**
All modes are configured in **Settings â†’ AI â†’ Embedding Backfill**:
...
**Configuration:**
Adjust timing in **Settings â†’ General â†’ Heartbeat**:
...
#### Embedding Provider Options ðŸš€
...
Settings â†’ AI Provider â†’ Embedding Provider (new section)
...
- Settings â†’ Rules page removed from navigation
...
- History shows: "âœ“ Classified To: Anime Movies" (misleading)
...
**Problem:**
Users with Ollama running on a remote host (e.g., `192.168.50.95:11434`) found that embeddings were still being sent to `localhost:11434` even after configuring the remote host in Settings â†’ AI Provider. This caused:

- âœ… `curl http://192.168.50.95:11434/api/embeddings` â†’ Works (external Ollama)
- âŒ `curl http://localhost:11434/api/embeddings` â†’ Fails (nothing inside container)
- âŒ RAG status shows 0 embeddings despite thousands of classifications
...
**Impact:**
RAG embeddings now correctly use your configured Ollama host. When you update the Ollama host in Settings â†’ AI Provider, the change takes effect immediately. Performance is maintained through intelligent caching with proper invalidation.
...
**Before:** Click "Change Settings" â†’ See empty dropdown â†’ Click "Test Connection" â†’ Wait â†’ Finally see profiles
**After:** Click "Change Settings" â†’ Profiles automatically load â†’ Ready to edit immediately
...
- Warning shows: "âš ï¸ Your [Radarr/Sonarr] configuration is missing a Quality Profile. Content won't be added until you select one."
...
1. **Priority System** (most reliable source wins):
   ...
   - Third: Normalized age-based rating (13â†’PG-13, 16â†’R, etc.)
...
3. **Original Ratings Preserved**:
   ...
   - Nothing is lostâ€”you can always see what it was
...
**Admin UI Panel** (Settings â†’ Metadata â†’ Rating Normalization):
...
- Library profiles show what's _actually in_ your libraries (e.g., "99% Comedy, 37% TV-MA")
...
A new TV-MA Comedy special will get high confidence even without specific presets, because it _looks like_ what's already there.
...
| Before                                                                                     | After                                                                            |
| ------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------- |
| Click "Add Presets" â†’ Separate popup â†’ Select presets â†’ Close â†’ Can't see Combined Signals | Click "Configure" â†’ Select presets inline â†’ Combined Signals visible immediately |
...
| Library dropdown (can change)                                                              | Read-only library header with lock icon ðŸ”’                                       |
...
- **Browse All Presets**: New dedicated page to browse all 168 system presets and manage your custom presets
  - Access via Classification â†’ Presets in the sidebar
...
- **Better Visual Feedback**: Selected presets now show green checkmarks (âœ“) instead of blue highlights, making it easier to see what you've chosen at a glance
...
- **Lock Icon Indicator**: Read-only library field now shows a lock icon (ðŸ”’) to clearly indicate it cannot be changed
...
Existing Overseerr configurations will continue to work. For improved accuracy, update your webhook JSON payload in Overseerr to use the new template from Settings â†’ Webhooks.
...
1. ðŸ’¥ **Configuration not saving properly** - Settings would revert to "Unknown" after save
2. ðŸ“› **"Connection Failed" error** - Shown immediately after saving valid configuration
3. â“ **"Unable to fetch" display** - Server and channel names wouldn't load in view mode
4. âš ï¸ **No success feedback** - Test connection wouldn't show success message in edit mode
...
âœ… **Configuration saves and persists correctly**
...
âœ… **Server and channel names display correctly**
...
- **Discord "Unknown" Display Issue**: Fixed issue where Discord settings page would show "Unknown" for server and channel names after saving configuration
  - âœ… **No more 500 errors** when fetching channel details
  - ðŸ”„ **Graceful fallback** with clear error messages when Discord API is unavailable
  - â±ï¸ **10-second timeout** prevents indefinite waiting on Discord client login
  - ðŸ“ **Better error messages** help identify configuration problems
  - ðŸ›¡ï¸ **Warning status** shows when fallback data is being used
...
- **Enhanced Discord Test Connection**: Test your Discord bot setup more thoroughly
  - ðŸŽ¯ **Sends actual test notification** to your Discord channel to verify setup
  - âœ… **Permission validation** - shows which permissions are granted or missing
  - ðŸ“Š **Detailed status feedback** - see exactly what's working and what needs attention
...
- **Test Notification**: Look for the green "âœ… Classifarr Test Notification" message in your Discord channel when testing
...
- **Automatic Exclusions**: Identifies what's _not_ in your library
...
- **Editable Signals**: Remove base signals (âœ•), restore removed (â†©), add new (+)
...
```
Item Arrives â†’ Check Authoritative Signals (100% match)
            â†’ Evaluate All Policies
            â†’ Score: Presets + Patterns + RAG + History
            â†’ Apply Weights â†’ Rank Results
            â†’ Determine Action (auto/prompt/manual)
```
...
- **â‰¥85% confidence** â†’ Auto-classify immediately (no AI call)
- **60-84% confidence** â†’ Prompt user via Discord (no AI call)
- **<60% confidence** â†’ Use AI to help choose (existing behavior)

**Benefits:**

- âš¡ **2-5 second latency improvement** per classification
- ðŸ’° **70-80% reduction in AI API costs**
- ðŸŽ¯ **More consistent results** from deterministic rules
- ðŸ“Š **Transparent scoring** - see full PolicyEngine breakdown

**Example:**

```
Before: PolicyEngine (300ms) â†’ AI Verification (3s) = 3.3s total
After:  PolicyEngine (300ms) â†’ Auto-classify = 0.3s total
```
...
- ðŸŽ„ **Holiday & Seasonal** - Christmas, Halloween, seasonal content
- ðŸˆ **Sports & Athletics** - NFL, NBA, Olympics, sports docs
- ðŸ¥Š **PPV & Combat Sports** - UFC, MMA, boxing, wrestling
- ðŸŽµ **Concert & Live Music** - Concerts, festivals, live performances
- ðŸŽ¤ **Stand-up Comedy** - Comedy specials and stand-up
- ðŸ† **Awards & Ceremonies** - Oscars, Emmys, award shows

**Benefits:**

- âœ… **Unified system** - Events use same flow as all content
- âš™ï¸ **Configurable** - Adjust keywords, weights via UI
- ðŸ”§ **Extensible** - Easy to add new event types
- ðŸ“ˆ **Better accuracy** - Can combine with other signals
...
- Low confidence breakdown (âœ… matching, âš ï¸ conflicting, â“ unknown)
- Close race comparisons
- New discovery handling (unknown studios)
- Pattern learning options ("Remember: A24 â†’ Indie")
...
| `policy_presets`      | Junction: policies â†” presets   |
...
- AI validation now skipped for high-confidence classifications (â‰¥85%)
...
```
Final Score = (Preset Ã— 0.40) + (Pattern Ã— 0.25) + (RAG Ã— 0.20) + (History Ã— 0.15)
