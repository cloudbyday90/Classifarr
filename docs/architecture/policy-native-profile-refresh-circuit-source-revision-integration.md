...
Add a lease-expiry reclaim integration case. It must prove that a later worker
safely reclaims a probe after an abandoned claim expires and that the old token
cannot complete or clear the circuit. Worker concurrency is documented in
[Native Profile Refresh Circuit Concurrent-Worker
