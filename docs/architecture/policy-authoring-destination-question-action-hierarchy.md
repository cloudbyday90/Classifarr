...
`ReadinessNextActionCard`. An absent optional native-recovery projection is
treated as no recovery action, so the component remains safe to render while
workflow data is still being assembled.
