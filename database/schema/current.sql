...
--
-- Name: enforce_database_health_transition_receipts_append_only(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.enforce_database_health_transition_receipts_append_only() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
...
--
-- Name: automatic_destination_evaluation; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.automatic_destination_evaluation (
    singleton boolean DEFAULT true NOT NULL,
    ...
    failure_code text,
    CONSTRAINT automatic_destination_evaluation_check CHECK ((((status = 'complete'::text) AND (input_fingerprint IS NOT NULL) AND (report IS NOT NULL) AND (evaluated_at IS NOT NULL) AND (failure_code IS NULL) AND (failure_count = 0)) OR ((status = 'failed'::text) AND (input_fingerprint IS NULL) AND (report IS NULL) AND (evaluated_at IS NULL) AND (failure_code IS NOT NULL) AND (failure_count > 0)))),
    CONSTRAINT automatic_destination_evaluation_check1 CHECK (((next_check_at > observed_at) AND ((evaluated_at IS NULL) OR (evaluated_at <= observed_at)))),
    CONSTRAINT automatic_destination_evaluation_evaluated_at_check CHECK (isfinite(evaluated_at)),
    CONSTRAINT automatic_destination_evaluation_failure_code_check CHECK ((failure_code = ANY (ARRAY['evidence_budget'::text, 'invalid_feedback'::text, 'evaluation_unavailable'::text]))),
    CONSTRAINT automatic_destination_evaluation_failure_count_check CHECK (((failure_count >= 0) AND (failure_count <= 5))),
    CONSTRAINT automatic_destination_evaluation_input_fingerprint_check CHECK ((input_fingerprint ~ '^[a-f0-9]{64}$'::text)),
    CONSTRAINT automatic_destination_evaluation_next_check_at_check CHECK (isfinite(next_check_at)),
    CONSTRAINT automatic_destination_evaluation_observed_at_check CHECK (isfinite(observed_at)),
    CONSTRAINT automatic_destination_evaluation_report_check CHECK (((jsonb_typeof(report) = 'object'::text) AND (octet_length((report)::text) <= 16384))),
    CONSTRAINT automatic_destination_evaluation_singleton_check CHECK (singleton),
    CONSTRAINT automatic_destination_evaluation_status_check CHECK ((status = ANY (ARRAY['complete'::text, 'failed'::text])))
);


--
-- Name: TABLE automatic_destination_evaluation; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: database_health_transition_receipts; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.database_health_transition_receipts (
    id bigint NOT NULL,
    receipt_version smallint DEFAULT 1 NOT NULL,
    statistics_reset_at timestamp with time zone CONSTRAINT database_health_transition_receipt_statistics_reset_at_not_null NOT NULL,
    before_read_operations character varying(16) CONSTRAINT database_health_transition_rece_before_read_operations_not_null NOT NULL,
    before_write_operations character varying(16) CONSTRAINT database_health_transition_rec_before_write_operations_not_null NOT NULL,
    before_cache_hits character varying(16) NOT NULL,
    before_estimated_dead_tuples character varying(16) CONSTRAINT database_health_transition__before_estimated_dead_tupl_not_null NOT NULL,
    before_tables_with_dead_tuples character varying(16) CONSTRAINT database_health_transition__before_tables_with_dead_tu_not_null NOT NULL,
    after_read_operations character varying(16) CONSTRAINT database_health_transition_recei_after_read_operations_not_null NOT NULL,
    after_write_operations character varying(16) CONSTRAINT database_health_transition_rece_after_write_operations_not_null NOT NULL,
    after_cache_hits character varying(16) NOT NULL,
    after_estimated_dead_tuples character varying(16) CONSTRAINT database_health_transition__after_estimated_dead_tuple_not_null NOT NULL,
    after_tables_with_dead_tuples character varying(16) CONSTRAINT database_health_transition__after_tables_with_dead_tup_not_null NOT NULL,
    confirmation_observation_count smallint DEFAULT 2 CONSTRAINT database_health_transition__confirmation_observation_c_not_null NOT NULL,
    recorded_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT database_health_transition_receipts_after_bucket_ck CHECK ((((after_read_operations)::text = ANY (ARRAY[('none'::character varying)::text, ('low'::character varying)::text, ('moderate'::character varying)::text, ('high'::character varying)::text])) AND ((after_write_operations)::text = ANY (ARRAY[('none'::character varying)::text, ('low'::character varying)::text, ('moderate'::character varying)::text, ('high'::character varying)::text])) AND ((after_cache_hits)::text = ANY (ARRAY[('none'::character varying)::text, ('low'::character varying)::text, ('moderate'::character varying)::text, ('high'::character varying)::text])) AND ((after_estimated_dead_tuples)::text = ANY (ARRAY[('none'::character varying)::text, ('low'::character varying)::text, ('moderate'::character varying)::text, ('high'::character varying)::text])) AND ((after_tables_with_dead_tuples)::text = ANY (ARRAY[('none'::character varying)::text, ('low'::character varying)::text, ('moderate'::character varying)::text, ('high'::character varying)::text])))),
    CONSTRAINT database_health_transition_receipts_before_bucket_ck CHECK ((((before_read_operations)::text = ANY (ARRAY[('none'::character varying)::text, ('low'::character varying)::text, ('moderate'::character varying)::text, ('high'::character varying)::text])) AND ((before_write_operations)::text = ANY (ARRAY[('none'::character varying)::text, ('low'::character varying)::text, ('moderate'::character varying)::text, ('high'::character varying)::text])) AND ((before_cache_hits)::text = ANY (ARRAY[('none'::character varying)::text, ('low'::character varying)::text, ('moderate'::character varying)::text, ('high'::character varying)::text])) AND ((before_estimated_dead_tuples)::text = ANY (ARRAY[('none'::character varying)::text, ('low'::character varying)::text, ('moderate'::character varying)::text, ('high'::character varying)::text])) AND ((before_tables_with_dead_tuples)::text = ANY (ARRAY[('none'::character varying)::text, ('low'::character varying)::text, ('moderate'::character varying)::text, ('high'::character varying)::text])))),
    CONSTRAINT database_health_transition_receipts_changed_state_ck CHECK ((((before_read_operations)::text <> (after_read_operations)::text) OR ((before_write_operations)::text <> (after_write_operations)::text) OR ((before_cache_hits)::text <> (after_cache_hits)::text) OR ((before_estimated_dead_tuples)::text <> (after_estimated_dead_tuples)::text) OR ((before_tables_with_dead_tuples)::text <> (after_tables_with_dead_tuples)::text))),
...
--
-- Name: TABLE database_health_transition_receipts; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON TABLE public.database_health_transition_receipts IS 'Append-only, confirmed coarse PostgreSQL health bucket transitions; no raw counts, query, database, media, library, provider, configuration, policy, AI, label, or routing data.';


--
-- Name: database_health_transition_receipts_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--

ALTER TABLE public.database_health_transition_receipts ALTER COLUMN id ADD GENERATED ALWAYS AS IDENTITY (
    SEQUENCE NAME public.database_health_transition_receipts_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
--
-- Name: database_health_transition_state; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.database_health_transition_state (
    singleton smallint DEFAULT 1 NOT NULL,
    statistics_reset_at timestamp with time zone NOT NULL,
    stable_read_operations character varying(16) CONSTRAINT database_health_transition_stat_stable_read_operations_not_null NOT NULL,
    stable_write_operations character varying(16) CONSTRAINT database_health_transition_sta_stable_write_operations_not_null NOT NULL,
    stable_cache_hits character varying(16) NOT NULL,
    stable_estimated_dead_tuples character varying(16) CONSTRAINT database_health_transition__stable_estimated_dead_tupl_not_null NOT NULL,
    stable_tables_with_dead_tuples character varying(16) CONSTRAINT database_health_transition__stable_tables_with_dead_tu_not_null NOT NULL,
    pending_read_operations character varying(16),
    pending_write_operations character varying(16),
    pending_cache_hits character varying(16),
    pending_estimated_dead_tuples character varying(16),
    pending_tables_with_dead_tuples character varying(16),
    pending_observation_count smallint DEFAULT 0 CONSTRAINT database_health_transition_s_pending_observation_count_not_null NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT database_health_transition_state_pending_ck CHECK ((((pending_observation_count = 0) AND (pending_read_operations IS NULL) AND (pending_write_operations IS NULL) AND (pending_cache_hits IS NULL) AND (pending_estimated_dead_tuples IS NULL) AND (pending_tables_with_dead_tuples IS NULL)) OR ((pending_observation_count = 1) AND ((pending_read_operations)::text = ANY (ARRAY[('none'::character varying)::text, ('low'::character varying)::text, ('moderate'::character varying)::text, ('high'::character varying)::text])) AND ((pending_write_operations)::text = ANY (ARRAY[('none'::character varying)::text, ('low'::character varying)::text, ('moderate'::character varying)::text, ('high'::character varying)::text])) AND ((pending_cache_hits)::text = ANY (ARRAY[('none'::character varying)::text, ('low'::character varying)::text, ('moderate'::character varying)::text, ('high'::character varying)::text])) AND ((pending_estimated_dead_tuples)::text = ANY (ARRAY[('none'::character varying)::text, ('low'::character varying)::text, ('moderate'::character varying)::text, ('high'::character varying)::text])) AND ((pending_tables_with_dead_tuples)::text = ANY (ARRAY[('none'::character varying)::text, ('low'::character varying)::text, ('moderate'::character varying)::text, ('high'::character varying)::text]))))),
    CONSTRAINT database_health_transition_state_singleton_ck CHECK ((singleton = 1)),
    CONSTRAINT database_health_transition_state_stable_bucket_ck CHECK ((((stable_read_operations)::text = ANY (ARRAY[('none'::character varying)::text, ('low'::character varying)::text, ('moderate'::character varying)::text, ('high'::character varying)::text])) AND ((stable_write_operations)::text = ANY (ARRAY[('none'::character varying)::text, ('low'::character varying)::text, ('moderate'::character varying)::text, ('high'::character varying)::text])) AND ((stable_cache_hits)::text = ANY (ARRAY[('none'::character varying)::text, ('low'::character varying)::text, ('moderate'::character varying)::text, ('high'::character varying)::text])) AND ((stable_estimated_dead_tuples)::text = ANY (ARRAY[('none'::character varying)::text, ('low'::character varying)::text, ('moderate'::character varying)::text, ('high'::character varying)::text])) AND ((stable_tables_with_dead_tuples)::text = ANY (ARRAY[('none'::character varying)::text, ('low'::character varying)::text, ('moderate'::character varying)::text, ('high'::character varying)::text]))))
);


--
-- Name: TABLE database_health_transition_state; Type: COMMENT; Schema: public; Owner: -
--
...
CREATE TABLE public.queue_startup_performance_receipts (
    ...
    CONSTRAINT queue_startup_performance_receipts_count_bucket_chk CHECK ((((scanned_id_bucket)::text = ANY (ARRAY[('not_applicable'::character varying)::text, ('zero'::character varying)::text, ('1_to_99'::character varying)::text, ('100_to_999'::character varying)::text, ('1000_to_4999'::character varying)::text, ('5000_or_more'::character varying)::text])) AND ((candidate_count_bucket)::text = ANY (ARRAY[('not_applicable'::character varying)::text, ('zero'::character varying)::text, ('1_to_99'::character varying)::text, ('100_to_999'::character varying)::text, ('1000_to_4999'::character varying)::text, ('5000_or_more'::character varying)::text])))),
    CONSTRAINT queue_startup_performance_receipts_duration_bucket_chk CHECK (((duration_bucket)::text = ANY (ARRAY[('unavailable'::character varying)::text, ('under_5ms'::character varying)::text, ('5_to_24ms'::character varying)::text, ('25_to_99ms'::character varying)::text, ('100_to_499ms'::character varying)::text, ('500ms_or_more'::character varying)::text]))),
    CONSTRAINT queue_startup_performance_receipts_observation_count_chk CHECK ((observation_count > 0)),
    CONSTRAINT queue_startup_performance_receipts_operation_chk CHECK (((operation_id)::text = ANY (ARRAY[('queue_worker_health'::character varying)::text, ('queue_refill_candidates'::character varying)::text])))
);
...
--
-- Name: automatic_destination_evaluation automatic_destination_evaluation_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: database_health_transition_receipts database_health_transition_receipts_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.database_health_transition_receipts
    ADD CONSTRAINT database_health_transition_receipts_pkey PRIMARY KEY (id);


--
-- Name: database_health_transition_state database_health_transition_state_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: idx_database_health_transition_receipts_current_reset; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_database_health_transition_receipts_current_reset ON public.database_health_transition_receipts USING btree (statistics_reset_at, recorded_at DESC, id DESC);
...
CREATE INDEX idx_task_queue_health_active ON public.task_queue USING btree (status, started_at DESC) WHERE ((status)::text = ANY (ARRAY[('pending'::character varying)::text, ('processing'::character varying)::text]));
...
--
-- Name: database_health_transition_receipts database_health_transition_receipts_append_only; Type: TRIGGER; Schema: public; Owner: -
--
